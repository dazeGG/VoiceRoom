...
export function verifyEngagementCheckpoint(value, { expectedSha, resolveArtifact } = {}) {
  ...
  if (!GIT_SHA.test(value.codeSha || '')) throw new Error('G71 requires an immutable evaluated code SHA');
...
function cli() {
  ...
  const result = verifyEngagementCheckpoint(readExternalCliArtifact(process.argv).value, { expectedSha: process.env.GITHUB_SHA });
