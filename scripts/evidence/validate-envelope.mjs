...
export function validateEnvelope(envelope, expectedPhase) {
  ...
  const phaseFields = expectedPhase === "F9" ? ["f7Digest"] : expectedPhase === "F11"
    ? ["f7Digest", "f9Digest", "mergeSha", "terminalDevelopSha", "remoteDeleted", "mergedAt", "remoteDeletionObservedAt"]
    : [];
  ...
  if (expectedPhase === "F9") assert.match(envelope.f7Digest, DIGEST);
...
function runBootstrapFixture() {
  ...
  const f9 = { schemaVersion: 1, goal: "G01", phase: "F9", status: "GREEN", evidenceId: "approval-envelope.g01.json", sourceSha: head, digest: `sha256:${"2".repeat(64)}`, createdAt: "2026-07-18T00:01:00.000Z", f7Digest: f7.digest };
  const f11 = { schemaVersion: 1, goal: "G01", phase: "F11", status: "GREEN", evidenceId: "merge-envelope.g01.json", sourceSha: merge, digest: `sha256:${"3".repeat(64)}`, createdAt: "2026-07-18T00:04:00.000Z", f7Digest: f7.digest, f9Digest: f9.digest, mergeSha: merge, terminalDevelopSha: merge, remoteDeleted: true, mergedAt: "2026-07-18T00:02:00.000Z", remoteDeletionObservedAt: "2026-07-18T00:03:00.000Z" };
