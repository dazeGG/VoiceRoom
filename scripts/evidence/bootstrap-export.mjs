...
async function resolveAuthenticatedPriorFailure(api, repository, prepared, artifactPages, prPages) {
  ...
  const mergedCanonicalShas = new Set((prPages ?? []).flatMap((page) => Array.isArray(page) ? page : []).filter((pr) => pr.merged_at && /^feature\/2\.5\.0-g01-(?:canonical-evidence-bootstrap|postmerge-bootstrap-a[0-9]{2,})$/.test(pr.head?.ref ?? "")).map((pr) => pr.merge_commit_sha));
  const latestSelectionOrdinal = Math.max(0, ...artifacts.flatMap((artifact) => {
    const match = artifact.name?.match(/^g01-selection-(?:g01-)?(?:recovery-)?a([0-9]{2,})-run-[0-9]+-attempt-[0-9]+-head-([0-9a-f]{40})$/);
    return match && mergedCanonicalShas.has(match[2]) ? [Number(match[1])] : [];
  }));
  const ordinal = Math.max(0, ...artifacts.flatMap((artifact) => {
    const match = artifact.name?.match(/^g01-bootstrap-failure-g01-(?:recovery-)?a([0-9]{2,})-run-[0-9]+-attempt-[0-9]+-head-([0-9a-f]{40})-phase-(?:f11|selection)$/);
    return match && artifact.expired !== true && mergedCanonicalShas.has(match[2]) ? [Number(match[1])] : [];
  }));
  assert.ok(ordinal > latestSelectionOrdinal, "recovery requires an outstanding landed failure ordinal");
...
export function validateLivePublication(authority, prepared) {
