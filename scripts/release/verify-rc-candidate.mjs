...
import { assertEvidenceIdentity, readRepositoryArtifact, sha256 } from '../checkpoints/immutable-evidence.mjs';
...
export function verifyRcCandidate(policy, evidence, { expectedSha, resolveArtifact = (file) => readRepositoryArtifact(file).bytes } = {}) {
...
function cli() {
  ...
  process.stdout.write(`${JSON.stringify({ ok: true, ...verifyRcCandidate(policy, readRepositoryArtifact(process.argv[index + 1]).value, { expectedSha: process.env.GITHUB_SHA }) })}\n`);
}
