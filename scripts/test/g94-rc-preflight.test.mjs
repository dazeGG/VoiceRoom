...
function fixture() {
  ...
  const authorizedSourceSha = 'a'.repeat(40); const parents = [g93Authority.developSha, authorizedSourceSha];
  g93Authority.mergeMethodAuthority.sha256 = bind(g93Authority.mergeMethodAuthority.artifactPath, { id: 'owner-authority', releaseHeadSha:sourceSha, baseDevelopSha:g93Authority.developSha, authorizedSourceSha, parents, method:'merge', releaseOnly:true }).sha256;
  ...
  return { evidence, expectedSha: sourceSha, resolveArtifact: (path) => artifacts.get(path), resolveCommit: () => ({ sha:sourceSha, treeSha:candidate.treeSha, parents }) };
}
...
test('RC-A02 candidate mutation, stale artifacts, missing physical proof and side effects fail closed', () => {
  for (const mutate of [(v) => { v.evidence.g93Authority.developSha = 'a'.repeat(40); }, (v) => { v.evidence.candidates.push(structuredClone(v.evidence.candidates[0])); }, (v) => { v.evidence.candidates[0].digests.api = hash('9'); }, (v) => { v.evidence.matrices = v.evidence.matrices.filter((m) => m.id !== 'physical-desktop'); }, (v) => { v.evidence.matrices[0].sha256 = hash('e'); }, (v) => { v.evidence.reviews.architect.sha256 = hash('a'); }, (v) => { v.evidence.productionContacts = 1; }, (v) => { v.expectedSha = '9'.repeat(40); }, (v) => { v.resolveCommit = () => ({sha:v.expectedSha,treeSha:'9'.repeat(40),parents:[]}); }]) { const value = fixture(); mutate(value); assert.throws(() => verifyRcCandidate(policy, value.evidence, value)); }
});
test('RC-A05 squash/rebase or a hostile merge parent cannot authorize the release head', () => { for (const hostile of [{method:'squash'},{authorizedSourceSha:'9'.repeat(40)}]) { const value = fixture(); const ref = value.evidence.g93Authority.mergeMethodAuthority; const original = JSON.parse(value.resolveArtifact(ref.artifactPath)); const bytes = Buffer.from(JSON.stringify({...original,...hostile})); ref.sha256 = sha256(bytes); const resolve = value.resolveArtifact; value.resolveArtifact = (path) => path === ref.artifactPath ? bytes : resolve(path); assert.throws(() => verifyRcCandidate(policy, value.evidence, value), /stale or hostile|canonical authorized merge/); } });
test('RC-A03 invalidated candidate requires a new contiguous fully rebuilt candidate', () => { const value = fixture(); const old = { ...structuredClone(value.evidence.candidates[0]), status: 'invalidated', invalidatedBy: 'rc-2', rebuildRequired: true }; const next = { ...structuredClone(value.evidence.candidates[0]), id: 'rc-2', ordinal: 2, sourceSha: 'e'.repeat(40), treeSha: 'f'.repeat(40) }; value.evidence.candidates = [old, next]; value.expectedSha = next.sourceSha; const ref = value.evidence.g93Authority.mergeMethodAuthority; const originalAuthority = JSON.parse(value.resolveArtifact(ref.artifactPath)); const bytes = Buffer.from(JSON.stringify({ ...originalAuthority, releaseHeadSha:next.sourceSha })); ref.sha256 = sha256(bytes); const original = value.resolveArtifact; value.resolveArtifact = (path) => path === ref.artifactPath ? bytes : original(path); value.resolveCommit = () => ({sha:next.sourceSha,treeSha:next.treeSha,parents:originalAuthority.parents}); assert.throws(() => verifyRcCandidate(policy, value.evidence, value), /candidate manifest/); });
