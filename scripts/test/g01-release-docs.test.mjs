...
test("sparse commit-pulls PR metadata authenticates merges without a merged field and rejects hostile merge states", () => {
  const base = chain(); const sparseMerge = structuredClone(base.mergeAuthority); delete sparseMerge.pr.merged; sparseMerge.pr.merged_at = "2026-07-18T00:02:00Z";
  const f11 = buildF11Envelope(base.f7, base.f9, sparseMerge); assert.equal(f11.mergedAt, "2026-07-18T00:02:00.000Z"); const sparseSelection = structuredClone(base.selectionAuthority); delete sparseSelection.pr.merged; buildSelection(base.f7, base.f9, base.f11, sparseSelection);
  for (const mutate of [(pr) => { pr.state = "open"; }, (pr) => { pr.merged_at = null; }, (pr) => { pr.merge_commit_sha = "c".repeat(40); }, (pr) => { pr.merged = false; }]) { const authority = structuredClone(sparseMerge); mutate(authority.pr); assert.throws(() => buildF11Envelope(base.f7, base.f9, authority)); const selection = structuredClone(sparseSelection); mutate(selection.pr); assert.throws(() => buildSelection(base.f7, base.f9, base.f11, selection)); }
});
...
test("F7 seals four authenticated Actions jobs/check-runs only after completion", () => {
  ...
  const previousTimezone = process.env.TZ; process.env.TZ = "Asia/Yakutsk";
  try {
    ...
    const offset = structuredClone(secondPrecision);
    offset.jobPages[0].jobs[0].started_at = "2026-07-19T09:34:23+02:00";
    offset.jobPages[0].jobs[0].completed_at = "2026-07-19T09:34:26+02:00";
    const normalizedOffset = buildAuthenticatedF7Envelope(report, head, BRANCH, "2026-07-19T07:37:00.000Z", offset, f7Files(head));
    assert.equal(normalizedOffset.requiredGates[0].startedAt, "2026-07-19T07:34:23.000Z");
    assert.equal(normalizedOffset.requiredGates[0].completedAt, "2026-07-19T07:34:26.000Z");
    for (const field of ["started_at", "completed_at"]) {
      for (const value of ["2026-07-19T07:34:23", "not-a-timestamp"]) {
        const hostile = structuredClone(secondPrecision); hostile.jobPages[0].jobs[0][field] = value;
        assert.throws(() => buildAuthenticatedF7Envelope(report, head, BRANCH, "2026-07-19T07:37:00.000Z", hostile, f7Files(head)), new RegExp(`${field === "started_at" ? "startedAt" : "completedAt"} must`));
      }
    }
  } finally {
...
test("live preparation creates a provisional recovery branch with complete authenticated lineage and next ordinal", async () => {
  ...
  const originalPaginate = api.paginate.bind(api);
  api.paginate = async (endpoint) => {
    const pages = await originalPaginate(endpoint);
    if (!endpoint.includes("/artifacts")) return pages;
    pages[0].artifacts.push(
      { id: 4, name: `g01-bootstrap-preparation-g01-recovery-a03-run-30-attempt-1-head-${"c".repeat(40)}`, expired: false, workflow_run: { id: 30 } },
      { id: 5, name: `g01-bootstrap-preparation-g01-recovery-a04-run-40-attempt-1-head-${"d".repeat(40)}`, expired: false, workflow_run: { id: 40 } },
    );
    return pages;
  };
  ...
  assert.equal(result.prepared.attemptId, "g01-recovery-a05"); assert.equal(result.registries.attempts.state, "G01_PRE_BRANCH"); assert.equal(result.registries.recoveries.currentRecovery, null); assert.deepEqual(result.registries.recoveries.landedAncestors, [prior]);
...
test("artifactless backfill sidecar binds distinct producer, failed subject, F7/F9, checks, logs and zero-artifact capture", () => {
  const subject="b".repeat(40),producer="c".repeat(40),head="a".repeat(40),failure={schemaVersion:1,release:"2.5.0",status:"G01_LANDED_UNSEALED",attemptId:"g01-a19",evidenceId:"bootstrap-failure.g01-a19.json",failedPhase:"F11",baseSha:"9".repeat(40),parentSha:"9".repeat(40),terminalDevelopSha:subject,headSha:head,runId:90,runAttempt:1,createdAt:"2026-07-19T09:00:00.000Z",reason:"artifact absent",recoveryLineage:[]};
  const core={schemaVersion:1,kind:"g01-artifactless-run-backfill",repository:REPOSITORY,attemptId:failure.attemptId,evidenceId:failure.evidenceId,failedPhase:"F11",failurePayloadDigest:envelopePayloadDigest(failure),subjectRun:{id:90,runAttempt:1,workflowId:10,checkSuiteId:11,workflowName:"CI/CD",workflowPath:".github/workflows/ci.yml",repository:REPOSITORY,event:"push",headBranch:"develop",headSha:subject,status:"completed",conclusion:"failure"},subjectPr:{id:86,number:86,nodeId:"PR86",state:"closed",headSha:head,headBranch:BRANCH,baseSha:failure.baseSha,mergeSha:subject,mergedAt:"2026-07-19T08:57:34Z"},subjectJobs:[{id:1,name:"Lint, typecheck & build",status:"completed",conclusion:"success"},{id:2,name:"Tests",status:"completed",conclusion:"success"},{id:3,name:"G01 authenticated post-merge F11",status:"completed",conclusion:"failure"}],failedJobLogDigest:digest("1"),checkRuns:[{id:4,name:"Lint, typecheck & build",status:"completed",conclusion:"success",detailsUrl:"https://example/4",checkSuiteId:11},{id:5,name:"Tests",status:"completed",conclusion:"success",detailsUrl:"https://example/5",checkSuiteId:11},{id:6,name:"G01 authenticated post-merge F11",status:"completed",conclusion:"failure",detailsUrl:"https://example/6",checkSuiteId:11}],f7Artifact:{id:7,name:`g01-candidate-g01-a19-run-80-attempt-1-head-${head}`,archiveDigest:digest("2"),payloadDigest:digest("3"),runId:80,runAttempt:1,headSha:head},f9Artifact:{id:8,name:`g01-approval-g01-a19-run-80-attempt-1-head-${head}`,archiveDigest:digest("4"),payloadDigest:digest("5"),runId:80,runAttempt:1,headSha:head},absenceCapture:{observedAt:"2026-07-19T09:10:00Z",pageDigests:[digest("6")],paginationEndMarker:"gh-api--paginate-completed-no-next-page",matchingArtifactIds:[]},producer:{runId:91,runAttempt:1,workflowId:10,checkSuiteId:12,headSha:producer,headBranch:"feature/g01-artifactless-backfill",event:"pull_request",workflowName:"CI/CD",workflowPath:".github/workflows/ci.yml",jobId:9,jobName:"G01 authenticated artifactless failure backfill"},createdAt:"2026-07-19T09:11:00.000Z"};const authority={...core,digest:compactDigest(core)};validateArtifactlessBackfillAuthority(authority,failure);
  for(const mutate of [x=>x.subjectRun.id++,x=>x.producer.runId=90,x=>x.producer.headBranch="feature/substitute",x=>x.absenceCapture.matchingArtifactIds.push(1),x=>x.f9Artifact.runId++,x=>x.subjectJobs[2].conclusion="success",x=>x.checkRuns[2].checkSuiteId++,x=>x.failurePayloadDigest=digest("9")]){const hostile=structuredClone(authority);mutate(hostile);assert.throws(()=>validateArtifactlessBackfillAuthority(hostile,failure));}
  const record={failure,artifactId:10,artifactName:`g01-bootstrap-failure-g01-a19-run-90-attempt-1-head-${subject}-phase-f11`,artifactCreatedAt:"2026-07-19T09:12:00Z",archiveDigest:digest("7"),payloadDigest:envelopePayloadDigest(failure),run:run(90,subject,"develop","push","completed","failure",1),provenance:{kind:"artifactless-run-backfill",authorityDigest:authority.digest,producerRunId:91,producerRunAttempt:1,producerHeadSha:producer,recoveryTerminalDevelopSha:"d".repeat(40),subjectTerminalDevelopSha:subject}};assert.equal(selectCanonicalFailure([record],19,"d".repeat(40),REPOSITORY),record);
  for(const mutate of [x=>x.provenance.producerRunId=90,x=>x.provenance.subjectTerminalDevelopSha="e".repeat(40),x=>x.provenance.recoveryTerminalDevelopSha="e".repeat(40)]){const hostile=structuredClone(record);mutate(hostile);assert.throws(()=>selectCanonicalFailure([hostile],19,"d".repeat(40),REPOSITORY));}
});
...
test("authenticated artifact timestamps normalize GitHub seconds and offsets while malformed or timezone-less values fail closed", () => {
  ...
  for (const [mergedAt, expected] of [["2026-07-18T00:02:00Z", "2026-07-18T00:02:00.000Z"], ["2026-07-18T03:02:00+03:00", "2026-07-18T00:02:00.000Z"]]) {
    const authority = structuredClone(base.mergeAuthority);
    authority.pr.merged_at = mergedAt;
    assert.equal(buildF11Envelope(base.f7, base.f9, authority).mergedAt, expected);
  }
  for (const mergedAt of ["not-a-timestamp", "2026-07-18T00:02:00", "2026-07-18"]) {
    const authority = structuredClone(base.mergeAuthority);
    authority.pr.merged_at = mergedAt;
    assert.throws(() => buildF11Envelope(base.f7, base.f9, authority), /PR merged_at must/);
  }
...
test("workflow has reachable bounded premerge F9 and automatic merged-commit F11/selection with authenticated provenance", () => {
  ...
  for (const token of ["github.ref == 'refs/heads/develop'", "commits/$GITHUB_SHA/pulls", "merge_commit_sha===process.env.GITHUB_SHA", "actions/runs/$GITHUB_RUN_ID/jobs", "Lint, typecheck & build", "Tests", "workflowPath", "download-digest", "g01-merge-", "bootstrap-selection:", "bootstrap-failure.", "GITHUB_RUN_ATTEMPT", "bootstrap-selection.$ATTEMPT_ID.json", "artifact-digest", "selection_sha256", "recoveryLineage", "always() &&"]) assert.ok(post.includes(token), `missing lifecycle proof: ${token}`);
  ...
  assert.equal(parsed.jobs.deploy, undefined, "ordinary CI must not expose a production deploy job");
});
...
test("recovery publication accepts authenticated one-file delta and rejects hostile variants", () => {
  ...
  const generatedIndex = entries.findIndex(({ path }) => path.endsWith("/bootstrap-attempts.json")), unchangedIndex = entries.findIndex(({ path }) => path === ".github/workflows/ci.yml");
  const headTree = { sha: "d".repeat(40), truncated: false, tree: entries.map(({ path, mode, sha }) => ({ path, mode, sha, type: "blob" })) }, baseTree = structuredClone(headTree); baseTree.sha = "e".repeat(40); baseTree.tree[generatedIndex].sha = "f".repeat(40);
  ...
  const generated = structuredClone(authority), generatedBytes = Buffer.from("generated-registry-authority"), generatedSha = crypto.createHash("sha1").update(`blob ${generatedBytes.length}\0`).update(generatedBytes).digest("hex"); generated.headBlobs[generatedIndex] = { ...generated.headBlobs[generatedIndex], content: generatedBytes.toString("base64"), sha256: `sha256:${crypto.createHash("sha256").update(generatedBytes).digest("hex")}`, size: generatedBytes.length, sha: generatedSha }; generated.headTree.tree[generatedIndex].sha = generatedSha; assert.doesNotThrow(() => validateLivePublication(bindStatus(generated), prepared));
  const allowlistedButNotGenerated = structuredClone(authority); allowlistedButNotGenerated.baseTree.tree[unchangedIndex].sha = "1".repeat(40); assert.throws(() => validateLivePublication(bindStatus(allowlistedButNotGenerated), prepared), /non-generated path/);
  ...
  const missing = structuredClone(authority); missing.headBlobs.pop(); assert.throws(() => validateLivePublication(bindStatus(missing), prepared), /all 36/);
  const duplicate = structuredClone(authority); duplicate.headBlobs[1].path = duplicate.headBlobs[0].path; assert.throws(() => validateLivePublication(bindStatus(duplicate), prepared), /duplicates/);
  const digestMismatch = structuredClone(prepared); digestMismatch.replayRecord.candidateTree.paths[unchangedIndex].sha256 = digest("9"); assert.throws(() => validateLivePublication(bindStatus(structuredClone(authority)), digestMismatch), /prepared candidate digest/);
  const sizeMismatch = structuredClone(prepared); sizeMismatch.replayRecord.candidateTree.paths[unchangedIndex].size++; assert.throws(() => validateLivePublication(bindStatus(structuredClone(authority)), sizeMismatch), /prepared candidate size/);
  const checksumMismatch = structuredClone(authority); checksumMismatch.headBlobs[unchangedIndex].content = Buffer.from("bad").toString("base64"); assert.throws(() => validateLivePublication(bindStatus(checksumMismatch), prepared), /PR blob digest mismatch/);
  const modeMismatch = structuredClone(authority); modeMismatch.headTree.tree[generatedIndex].mode = "100755"; assert.throws(() => validateLivePublication(bindStatus(modeMismatch), prepared), /tree mode mismatch/);
  const emptyDelta = structuredClone(authority); emptyDelta.baseTree = structuredClone(emptyDelta.headTree); emptyDelta.baseCommit.commit.tree.sha = emptyDelta.baseTree.sha; assert.throws(() => validateLivePublication(bindStatus(emptyDelta), prepared), /recovery publication delta must be nonempty/);
});
