...
test("cleanup ref deletion is compare-and-delete and a SHA race aborts WAITING before branch creation", async () => {
  ...
  await assert.rejects(() => prepareAndCreateBootstrapBranch({ api, repository: REPOSITORY, registries, ...reviewedTree(), observedAt: "2026-07-18T00:00:00.000Z" }), /WAITING/); assert.equal(created, false);
});
...
test("live pre-branch orchestration rejects consumed direct history and ref-create races without publishing a consumed ordinal", async () => {
  ...
  let consumedTick = 0; const consumed = makeApi({ consumed: true }); await assert.rejects(() => prepareAndCreateBootstrapBranch({ api: consumed, repository: REPOSITORY, registries, ...reviewedTree(), observedAt: () => `2026-07-18T00:00:0${consumedTick++}.000Z` }), /prepared authority|WAITING|READY/); assert.equal(consumed.calls.some(([name]) => name === "createRef"), false);
...
test("workflow has reachable bounded premerge F9 and automatic merged-commit F11/selection with authenticated provenance", () => {
  ...
  assert.ok(read("scripts/evidence/bootstrap-export.mjs").includes("--prepare-live"), "live preparation CLI must exist"); assert.doesNotMatch(read("scripts/evidence/bootstrap-export.mjs"), /--prior-failure/, "arbitrary prior-failure JSON input is forbidden"); assert.doesNotMatch(workflow, /OMX_G01_PREPARED_AUTHORITY_JSON|PREPARED_AUTHORITY_JSON/, "mutable prepared-authority transport is forbidden");
