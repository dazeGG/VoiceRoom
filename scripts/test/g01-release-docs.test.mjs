...
test("recovery publication accepts authenticated one-file delta and rejects hostile variants", () => {
  ...
  const digestMismatch = structuredClone(prepared); digestMismatch.replayRecord.candidateTree.paths[0].sha256 = digest("9"); assert.throws(() => validateLivePublication(bindStatus(structuredClone(authority)), digestMismatch), /prepared candidate digest/);
  const sizeMismatch = structuredClone(prepared); sizeMismatch.replayRecord.candidateTree.paths[0].size++; assert.throws(() => validateLivePublication(bindStatus(structuredClone(authority)), sizeMismatch), /prepared candidate size/);
  const checksumMismatch = structuredClone(authority); checksumMismatch.headBlobs[0].content = Buffer.from("bad").toString("base64"); assert.throws(() => validateLivePublication(bindStatus(checksumMismatch), prepared), /PR blob digest mismatch/);
  const modeMismatch = structuredClone(authority); modeMismatch.headTree.tree[0].mode = "100755"; assert.throws(() => validateLivePublication(bindStatus(modeMismatch), prepared), /tree mode mismatch/);
