...
test('G92-A02 evidence rejects unknown, stale, mutable or unsafe matrices', () => {
  const matrix = buildActivationMatrix(manifest); const evidence = { contract: 'voice-room.activation-matrix/v1', release: '2.5.0', gitSha: 'a'.repeat(40), manifestSha256: manifestDigest(bytes), cases: [...matrix.cases.map((item) => ({ ...item, enabled: item.expectedEnabled ?? true, passed: true })), ...['provider-failure','worker-failure','livekit-failure','disk-failure','restart','stale-replica'].map((id) => ({ id, vector: id, passed: true, failClosed: true }))], evidenceChainSha256: '' };
