...
test('G62-A01 room policy API round-trips all mentions none without collapsing explicit all',async()=>{expect(api).toContain("'all' | 'mentions' | 'none'");expect(api).toContain("payload.level === 'all'");expect(api).toContain('setRoomNotificationLevel');});
