...
test('G84-A01 1-4 mosaics reserve dimensions, lazy-load and degrade deleted/unavailable images', async () => {
  expect(mosaic).toContain('attachments.slice(0, 4)');
  expect(mosaic).toContain("attachment.state === 'ready'");
  expect(mosaic).toContain('loading="lazy"');
  expect(mosaic).toContain('width={attachment.width} height={attachment.height}');
  expect(mosaic).toContain('Изображение недоступно');
  expect(css).toContain("data-count='3'");
...
test('G84-A02 lightbox traps/restores focus and supports Escape/arrows/zoom/authorized download', async () => {
  ...
  expect(lightbox).toContain('return () => previous?.focus()');
