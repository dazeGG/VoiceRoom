...
const PNG_BASE64 =
  'iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAACXBIWXMAAAPoAAAD6AG1e1JrAAAARUlEQVRYhe3XsREAMAhC0YzIOIzoVmaLpHmFvXcifE4z+3OOBeoEIcLxhsuIyoojjEYcLyApJAsoHVi+iklVsyin81wHF4vcPJcf4sTZAAAAAElFTkSuQmCC';
...
test('image attachments support picker, removal, drag-and-drop, paste, and sending', async ({ page }) => {
  ...
  const pickerUpload = page.waitForResponse(
    (response) =>
      /\/api\/media\/attachments\/[^/]+\/content$/.test(response.url()) && response.request().method() === 'PUT'
  );
  ...
  const deleteResponse = page.waitForResponse(
    (response) => /\/api\/media\/attachments\/[^/]+$/.test(response.url()) && response.request().method() === 'DELETE'
  );
  ...
  const slotResponse = page.waitForResponse(
    (response) => response.url().endsWith('/api/media/attachments') && response.request().method() === 'POST'
  );
  ...
  const pasteSlot = page.waitForResponse(
    (response) => response.url().endsWith('/api/media/attachments') && response.request().method() === 'POST'
  );
