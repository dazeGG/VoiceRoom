...
export async function buildEmojiAssets({ outputDir = defaultOutputDir, catalogueFile = defaultCatalogueFile } = {}) {
  ...
  await Promise.all(
    copies.map(({ source, name }) => copyFile(join(packageRoot, `${source}.svg`), join(outputDir, `${name}.svg`)))
  );
  ...
  await writeFile(
    ...
    [
      ...
      '',
      'Generated by apps/web/scripts/build-emoji-assets.ts. Do not edit by hand.',
      ''
    ].join('\n'),
    'utf8'
  );
  ...
  const catalogue = {
    _comment:
      'Generated by apps/web/scripts/build-emoji-assets.ts. The emoji this app draws and offers — ' +
