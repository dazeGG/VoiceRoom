...
${read('src/lib/features/room/components/RoomChatPanel.svelte')}`;
  ...
  assert.match(previewChat, /message\.peerId !== peerId && !isRoomNotificationsMuted\(roomId\)[\s\S]*playRoomChatMessageCue\(message\.id\)/);
  assert.match(roomChat, /isRoomNotificationsMuted\(roomId\)/);
  assert.match(roomChat, /message\.peerId !== peerId && !isRoomNotificationsMuted\(roomId\)[\s\S]*playRoomChatMessageCue\(message\.id\)/);
