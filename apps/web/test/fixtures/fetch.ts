...
export type Reply = { status?: number; body?: unknown } | ((init: RequestInit | undefined) => { status?: number; body?: unknown });
...
export function stubFetch(routes: Record<string, Reply>) {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!route) return new Response(JSON.stringify({ ok: false, error: `no stub for ${method} ${url}` }), { status: 500 });
