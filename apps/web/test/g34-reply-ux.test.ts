...
test('G34-A01 reply submit retains draft on error and clears it on success', async () => {
  ...
  assert.equal(await store.submit(async (input: { replyTo?: unknown }) => assert.deepEqual(input.replyTo, { messageId: 'm' })), true); assert.equal(store.draft, ''); assert.equal(store.target, null);
});
