...
export async function loadLiveKitHarness(options: {
  ...
  vi.doMock(`${P}/media/profiles`, () => ({ getScreenProfile: () => ({ id: 'balanced-30' }), getScreenPublishVideoOptions: () => ({}) }));
  vi.doMock(`${P}/media/livekit-runtime`, () => ({
    ...
    loadLiveKitClient: () => options.autoResolveClient
      ? Promise.resolve(livekitClient)
      : new Promise((resolve) => { livekitClientResolvers.push(() => resolve(livekitClient)); })
  }));
  vi.doMock(`${P}/media/screen-subscription-retry`, () => ({
    createScreenSubscriptionRetryController: () => ({
      clear: () => { retries.cleared += 1; },
      clearAll: () => { retries.clearedAll += 1; },
      schedule: (input: { peerId: string }) => { retries.scheduled.push(input); }
    })
  }));
  vi.doMock(`${P}/room/participants`, () => ({
    createParticipant: (peerInfo: Peer) => {
      ...
      if (existing) {
        if (Object.hasOwn(peerInfo, 'screen') && existing.screenAuthoritative !== false) existing.screen = Boolean(peerInfo.screen);
        if (Object.hasOwn(peerInfo, 'screenAudio') && existing.screenAuthoritative !== false) existing.screenAudio = Boolean(peerInfo.screenAudio);
        return existing;
      }
    ...
    attachRemoteTrack: () => {},
    ensureRemoteAudioElement: (peer: Peer) => { audio.ensured.push({ peerId: peer.id }); return null; },
...
export function fakePublication(source: 'microphone' | 'screen-video' | 'screen-audio', extra: Record<string, unknown> = {}) {
...
export function fakeParticipant(identity: string, publications: Array<{ trackSid: string }>, extra: Record<string, unknown> = {}) {
  return {
    identity,
    isLocal: false,
    isScreenShareEnabled: publications.some((publication) => (publication as { source?: string }).source === 'screen-video'),
    joinedAt: new Date(0),
    name: identity,
