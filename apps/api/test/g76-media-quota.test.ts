...
test(
  'G76-A01 concurrent ninth slot is rejected and retry is idempotent',
  ...
  async (t) => {
    ...
    await pool.query(
      `INSERT INTO users(id,login,display_name,password_hash) VALUES ('quota-owner','quota-owner','Owner','x')`
    );
    const attachments = createAttachmentRepository({ pool });
    const quota = createMediaQuotaService({
      attachmentRepository: attachments,
      quotaRepository: createMediaQuotaRepository({ attachmentRepository: attachments, pool })
    });
    const results = await Promise.allSettled(
      Array.from({ length: 9 }, (_, index) =>
        quota.reserve({ ownerId: 'quota-owner', context: 'room', clientRequestId: `request-${index}`, bytes: 1024 })
      )
    );
