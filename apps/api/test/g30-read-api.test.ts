...
test('G30-A01 room read uses an exact purpose-bound tuple and reports monotonic no-op', async () => {
  ...
  const service = createMessageReadService({
    ...
    cursorCodec: codec,
    repository: fake<MessageReadRepository>({
      advanceRoom: async (input) => {
        tuple = input.tuple;
  ...
  assert.deepEqual(await service.advanceRoom({ roomId: 'r', userId: 'u', cursor }), {
    advanced: false,
    cursor,
    readThrough: null
  });
...
test('G30-A01b an advancing room read reports the instant it read through', async () => {
  ...
  const service = createMessageReadService({
    ...
    cursorCodec: codec,
    repository: fake<MessageReadRepository>({
      advanceRoom: async () => ({ last_read_message_created_at: readAt, last_read_message_id: 'm9' })
    })
  });
  assert.deepEqual(await service.advanceRoom({ roomId: 'r', userId: 'u', cursor }), {
    advanced: true,
...
test('G30-A02 wrong-purpose and forbidden room reads fail before mutation', async () => {
  ...
  await assert.rejects(service.advanceRoom({ roomId: 'r', userId: 'u', cursor: 'opaque' }), {
    code: 'room_forbidden',
    statusCode: 403
  });
