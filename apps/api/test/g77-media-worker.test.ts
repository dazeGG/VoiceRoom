...
test('G77-A03 a claimed upload is turned upright into a WebP image and a preview, then completed', async () => {
  ...
  const original = await sharp({ create: { width: 40, height: 20, channels: 3, background: '#3366cc' } }).jpeg().withMetadata({ orientation: 6 }).toBuffer();
  const saved: Record<string, Buffer> = {};
  let completed: Record<string, unknown> | null = null;
  const worker = createMediaProcessingWorker({
    attachmentRepository: { async findById() { return { internalState: 'processing' }; } },
    storage: {
      async openRead() { return { stream: (async function* () { yield original; })() }; },
      async save(id: string, variant: string, bytes: Buffer) { saved[variant] = bytes; return { key: `${id}/${variant}`, bytes: bytes.length }; }
    },
    jobRepository: {
      async oldestPendingAgeMs() { return 0; },
      async claimBatch() { return [{ id: 'job', attachmentId: 'att', attempts: 1, fencingToken: 1, createdAt: new Date() }]; },
      async renew() {},
      async completeProcessing(jobId: string, input: { attachmentResult: Record<string, unknown> }) { completed = { jobId, ...input.attachmentResult }; }
    },
  ...
  assert.deepEqual(completed, { jobId: 'job', processedStorageKey: 'att/processed', previewStorageKey: 'att/preview', processedBytes: saved.processed?.length, previewBytes: saved.preview?.length });
});
