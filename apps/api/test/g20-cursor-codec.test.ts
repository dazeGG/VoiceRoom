...
test('G20-A01 preserves exact microsecond tuples and accepts the previous rotation key', () => {
  ...
  const cursor = old.encode({
    purpose: 'room-history',
    context: 'room:a',
    tuple: { createdAtMicros: '1725000000000001', id: 'm2' }
  });
  assert.deepEqual(rotated.decode(cursor, { purpose: 'room-history', context: 'room:a' }), {
    createdAtMicros: '1725000000000001',
    id: 'm2'
  });
});
...
test('G20-A02 tamper, expiry, purpose and context mismatches have one non-oracle error', () => {
  ...
  for (const [value, expected] of invalid)
    assert.throws(
      ...
      (error) => error instanceof CursorCodecError && error.code === 'invalid_cursor' && error.statusCode === 400
    );
  now = 1011;
  assert.throws(() => codec.decode(cursor, { purpose: 'room-history', context: 'room:a' }), {
    code: 'invalid_cursor',
    statusCode: 400
  });
});
