...
test(
  'bookmark backfill promotes list-only users to members, skips bans and dead rooms, and rolls back only its rows',
  ...
  async (t) => {
    ...
    for (const [roomId, login] of bookmarks) {
      await pool.query(`INSERT INTO room_bookmarks (id, room_id, user_id, created_at) VALUES ($1, $2, $3, $4)`, [
        crypto.randomUUID(),
        roomId,
        ids[login],
        bookmarkedAt
    ...
    assert.deepEqual(
      promoted.map((row) => [row.login, row.role]),
      [
        ['alice', 'member'],
        ['carol', 'member'],
        ['dave', 'member'],
    ...
    await step(db.databaseUrl, 'down');
    assert.deepEqual(
      (await membershipsOf(pool, 'static-room')).map((row) => row.login),
      ['erin', 'owner']
    );
