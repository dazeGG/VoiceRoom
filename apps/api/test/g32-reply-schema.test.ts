...
import {
  ...
  requireReplyTarget
} from '../src/domains/messaging/reply-projector.ts';

test('G32-A01 reply projection is non-recursive and terminal states tombstone', async () => {
  const preview = projectReplyPreview({
    id: 'reply',
    text: ' child ',
    replyTo: { messageId: 'root' },
    replyPreview: { messageId: 'root' }
  });
  ...
  await assert.rejects(
    requireReplyTarget({ message: { id: 'invite', metadata: { kind: 'room-invite' } }, visibility: true }),
...
test('G32-A02 schema keeps immutable pointers without purge-cascade foreign keys', () => {
  const source = fs.readFileSync(
    path.resolve(import.meta.dirname, '../src/migrations/20260718123000_add_message_replies.cjs'),
    'utf8'
  );
