...
test('G53-A02 room append owns one injected transaction callback',()=>{const s=fs.readFileSync(path.resolve(import.meta.dirname,'../src/server.js'),'utf8');const start=s.indexOf('getMessageService().room.appendMessage(roomId');const end=s.indexOf('\n  });',start);const call=s.slice(start,end);assert.match(call,/unitOfWork:[\s\S]*\? async \(client, inserted\)/);assert.equal((call.match(/async \(client, inserted\)/g)||[]).length,1);});
