...
// cursor would).
test('G24-A02 room and DM history page through equal-time messages without gaps or repeats', { skip: !process.env.TEST_DATABASE_URL }, async (t) => {
  ...
  await pool.query(`INSERT INTO users (id, login, display_name, password_hash) VALUES ('a', 'g24a', 'A', 'x'), ('b', 'g24b', 'B', 'x')`);
  await pool.query(`INSERT INTO rooms (id, creator_ip) VALUES ('room', '')`);
  for (const id of ids) {
    await pool.query(`INSERT INTO room_messages (id, room_id, peer_id, name, text, created_at) VALUES ($1, 'room', 'p', 'A', $2, $3)`, [id, id, at]);
    await pool.query(`INSERT INTO direct_messages (id, sender_id, recipient_id, body, created_at) VALUES ($1, 'a', 'b', $2, $3)`, [id, id, at]);
  }
  ...
  while (page.hasMoreBefore) {
    ...
    page = await rooms.listBefore({ roomId: 'room', anchor: { id: first.id, createdAtMicros: first.createdAtMicros }, limit: 3, now });
    seenRoom.unshift(...page.messages.map((message) => message.id));
  }
  ...
  while (dmPage.hasMoreBefore) {
    ...
    dmPage = await dms.listBefore({ userId: 'a', peerId: 'b', anchor: { id: first.id, createdAtMicros: first.createdAtMicros }, limit: 3 });
    seenDm.unshift(...dmPage.messages.map((message) => message.id));
  }
