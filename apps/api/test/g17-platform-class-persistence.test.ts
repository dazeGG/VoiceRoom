...
test(
  'G17-A01 fresh migration is idempotent and application rollback preserves N-1 reads',
  ...
  async (t) => {
    ...
    await withClient(databaseUrl, async (client) => {
      const {
        ...
        `SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'push_subscriptions' AND column_name = 'platform_class') AS column_exists`
      );
...
test(
  'G17-A01 upgrade reclassifies 10k rows within five seconds and removes every raw platform signal',
  { skip: !process.env.TEST_DATABASE_URL },
  async (t) => {
    const { cleanup, databaseUrl } = await createTestDatabase(t);
    t.after(cleanup);
    ...
    await withClient(databaseUrl, async (client) => {
      ...
      await client.query(
        `INSERT INTO users (id, login, display_name, password_hash) VALUES ($1, 'g17user', 'G17 User', 'fixture')`,
        [userId]
      );
      await client.query(
        `
    ...
    await withClient(databaseUrl, async (client) => {
      const {
        ...
        `SELECT count(*)::int AS total, count(*) FILTER (WHERE platform_class = 'mobile')::int AS mobile, count(*) FILTER (WHERE platform_class = 'desktop')::int AS desktop, count(*) FILTER (WHERE platform_class = 'unknown')::int AS unknown FROM push_subscriptions`
      );
      assert.deepEqual(counts, { total: 10000, mobile: 4286, desktop: 4286, unknown: 1428 });
      const {
        rows: [{ raw_count }]
      } = await client.query(`SELECT count(*)::int AS raw_count FROM push_subscriptions WHERE metadata ?| $1::text[]`, [
        SIGNAL_KEYS
      ]);
      assert.equal(raw_count, 0);
      const {
        rows: [{ retained }]
      } = await client.query(`SELECT metadata->>'retained' AS retained FROM push_subscriptions WHERE id = 'g17-3'`);
...
test(
  ...
  async (t) => {
    ...
    await withClient(databaseUrl, async (client) => {
      await client.query(
        `INSERT INTO users (id, login, display_name, password_hash) VALUES ('g17-store-user', 'g17store', 'G17 Store', 'fixture')`
      );
    });
    for (const [index, signals] of corpus.entries()) {
      const stored = await store.upsert({
        ...
        subscription: {
          endpoint: `https://push.example/corpus-${index}`,
...
test(
  'G17-A02 lock timeout rolls the target migration back in at most five seconds',
  { skip: !process.env.TEST_DATABASE_URL },
  async (t) => {
    const migrationSource = fs.readFileSync(
      path.join(MIGRATIONS_DIR, '20260720161000_reclassify_push_subscription_platform.cjs'),
      'utf8'
    );
    ...
    await withClient(databaseUrl, (client) =>
      client.query(
        `INSERT INTO users (id,login,display_name,password_hash) VALUES ('locked-user','locked-user','Locked','x'); INSERT INTO push_subscriptions (id,user_id,endpoint,p256dh,auth,metadata) VALUES ('locked-sub','locked-user','https://push.example/locked','k','a','{"desktopBridge":true}')`
      )
    );
    ...
    assert.ok(
      ...
      `five-second database lock timeout completed with runner overhead in ${durationMs.toFixed(1)}ms`
    );
    ...
    await withClient(databaseUrl, async (client) => {
      ...
      const {
        ...
        `SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'push_subscription_platform_class') AS type_exists`
      );
      ...
      const {
        ...
        `SELECT metadata ? 'desktopBridge' AS has_signal FROM push_subscriptions WHERE id='locked-sub'`
      );
