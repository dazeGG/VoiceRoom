...
function transport(id) {
  const sent = [];
  return { id, sent, send(message) { sent.push(message.type); return true; } };
}

function presence() {
  return createRoomPresence({
    store: () => ({ async markRoomActive() {}, async markRoomEmpty() {}, async pruneRooms() {}, async getRoom() { return null; } }),
    runtime: () => null,
    logger: () => ({ error() {} }),
...
function roomHarness({ runtime = true, invitations = true, leaseError = false, revokeFails = false, removeFails = false } = {}) {
  const calls = { mirrored: [], invalidated: [], summaries: [], notified: [], revoked: [], removed: [], avatars: [], errors: [], warnings: [] };
  ...
  const lifecycle = createRoomLifecycle({
    ...
    runtime: () => (runtime ? {
      ...
      async cancelRoomReconnectLeases({ finalizePeer }) {
        const results = [];
        for (const [peer, ownershipFinalized] of [[null, false], [{ id: 'gone' }, true], ...[...roster.rooms.get('r1')?.peers.values() || []].map((p) => [p, false])]) {
    ...
    invitations: () => (invitations ? { async expirePendingInvites({ roomId }) { return roomId === 'r1' ? [{ senderId: 's', recipientId: 'r' }] : []; } } : null),
    notifyUser: (userId, event) => calls.notified.push([userId, event.type]),
    credentials: () => ({ async revokePeer(input) { if (revokeFails) throw new Error('revoke'); calls.revoked.push(input); } }),
    removeParticipant: async (roomId, peerId) => { if (removeFails) throw new Error('sfu'); calls.removed.push(peerId); },
    removeAvatar: async (key) => { calls.avatars.push(key); },
...
test('a profile change updates every seat the account holds', () => {
  ...
  assert.deepEqual([mine.name, mine.avatarUrl, mine.avatarAccent, mine.avatarColorKey], ['Ann', '/api/avatars/k.webp', 'gold', 'red']);
  lifecycle.refreshActiveProfile({ id: 'u1', avatarColorKey: 'blue' });
  assert.deepEqual([mine.name, mine.avatarUrl, mine.avatarAccent, mine.avatarColorKey], ['Nameless', null, null, 'blue']);
...
test('room invitations expire only with a store that can', async () => {
  ...
  assert.deepEqual(calls.notified, [['s', 'dm.message.edited'], ['r', 'dm.message.edited']]);
...
test('deleting a room tells everyone, frees every seat and removes the avatar', async () => {
  ...
  assert.deepEqual(calls.revoked, [{ roomId: 'r1', accountUserId: 'u1', guestPrincipalId: '' }, { roomId: 'r1', accountUserId: null, guestPrincipalId: 'g' }]);
...
test('a room deletion with failing cleanup still finishes and only warns', async () => {
  ...
  const warnings = [];
  await failing.lifecycle.finishRoomDeletion('r1', { request: { log: { warn: (fields) => warnings.push(fields.code), error() {} } } });
  ...
  const expiryFails = createRoomLifecycle({
    presence: presence(),
    runtime: () => null,
    invitations: () => ({ async expirePendingInvites() { throw new Error('db'); } }),
    notifyUser() {},
...
function accountHarness({ sockets = true, deletions = true, friendsFail = false, principal = true, voiceFails = false } = {}) {
  ...
  const lifecycle = createAccountLifecycle({
    ...
    revokeSeatCredentials: async (input) => { calls.revoked.push(input.peerId); },
    leaveVoice: async (connection, activeVoice) => { if (voiceFails) throw new Error('leave'); calls.left.push(activeVoice.peerId); },
    removeParticipant: async (roomId, peerId) => { calls.removed.push(peerId); },
    sessionRevokedCloseCode: 4401,
    deletions: () => (deletions ? {
      async listDueDeletions() { return ['kept', 'done', 'broken']; },
      async finalizeDeletion({ userId }) {
        ...
        return { status: 'deleted', avatarKey: 'a.webp', transferredRooms: [{ roomId: 'heir' }, { roomId: 'vanished' }], deletedRooms: [{ roomId: 'orphan', avatarKey: 'o.webp' }] };
      }
    } : null),
    findRoom: async (roomId) => (roomId === 'heir' ? { id: 'heir' } : null),
    announceRoomUpdate: (roomId) => calls.announced.push(roomId),
    finishRoomDeletion: async (roomId, options) => { calls.finished.push([roomId, options.avatarKey]); },
    removeAvatar: async (key) => { calls.avatars.push(key); },
    logger: () => ({ error: (fields) => calls.errors.push(fields.evt) })
  });
...
test('maintenance runs enabled tasks on its interval, logs failures and stops on close', async (t) => {
  ...
  const timer = startMaintenanceTimers(server, {
    keepaliveMs: 10,
    intervalMs: 100,
    pruneSockets: () => { calls.pruned += 1; if (calls.pruned === 2) throw new Error('ws'); },
    observe: async (name, run) => { calls.observed.push(name); return run(); },
    logger: { error: (fields, message) => calls.errors.push([fields.task, message]) },
    tasks: [
      { name: 'a', label: 'task-a', failureMessage: 'a failed', run: async () => { calls.ran.push('a'); } },
      { name: 'b', label: 'task-b', failureMessage: 'b failed', run: async () => { throw new Error('b'); } },
      { name: 'c', label: 'task-c', failureMessage: 'c failed', enabled: () => false, run: async () => { calls.ran.push('c'); } }
    ]
  });
  assert.ok(timer);
  ...
  assert.deepEqual(calls.errors, [['ws-prune', 'ws prune timer failed'], ['task-b', 'b failed']]);
  server.emit('close');
...
test('without a sweep interval only the socket reaper runs', (t) => {
  ...
  const server = new EventEmitter();
  let pruned = 0;
  assert.equal(startMaintenanceTimers(server, { keepaliveMs: 10, intervalMs: 0, pruneSockets: () => { pruned += 1; }, observe: async () => {}, logger: { error() {} }, tasks: [] }), null);
