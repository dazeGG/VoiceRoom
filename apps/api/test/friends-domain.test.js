...
function harness(store = {}, options = {}) {
  ...
  const friends = {
    async listFriends() { return [{ user: { id: 'f1' }, unreadCount: 2, lastMessage: null }, { user: { id: 'f2' }, friendsSince: 5 }]; },
    async countIncomingRequests() { return 3; },
    async searchUsers() { return [{ id: 'f1' }, { id: 'out' }, { id: 'in' }, { id: 'none' }]; },
    async getFriendIds() { return ['f1']; },
    async listRequests() { return { incoming: [{ user: { id: 'in' } }], outgoing: [{ user: { id: 'out' } }] }; },
    async sendRequest() { return { status: 'pending', user: { id: 'u2' }, requestId: 'r1' }; },
    async respondRequest({ action }) { return action === 'accept' ? { status: 'accepted', requesterId: 'u2', user: { id: 'u2' } } : { status: 'declined' }; },
    async cancelRequest() { return { status: 'cancelled', addresseeId: 'u2' }; },
    async removeFriend() { return { status: 'removed' }; },
    async listBlockedUserIds() { return ['b1']; },
    async listBlockedUsers() { return [{ id: 'b1' }]; },
    async blockUser() { return { status: 'blocked', unfriended: true }; },
    async unblockUser() { return { status: 'unblocked' }; },
    async areFriends() { return true; },
    async isBlockedBetween() { return false; },
    ...store
  };
  const service = createFriendsService({
    ...
    sendDirectMessage: async (input) => { calls.dms.push(input); return { id: 'dm-1' }; },
    isOnline: (userId) => userId === 'f1',
    notifyUser: (userId, event) => calls.events.push([userId, event.type]),
    queuePush: async (userId, payload, context) => { calls.pushes.push([userId, payload.type, payload.title, context]); },
...
test('lists, search relationships and requests', async () => {
  ...
  assert.deepEqual((await service.search('user-1', 'q')).map((row) => row.relationship), ['friend', 'outgoing', 'incoming', 'none']);
...
test('sending a request notifies the addressee; a crossing request accepts', async () => {
  const sent = harness();
  assert.deepEqual(await sent.service.sendRequest(ME, { userId: '', login: 'bob' }), { status: 'sent', user: { id: 'u2' } });
  assert.deepEqual(sent.calls.events, [['u2', 'friend-request'], ['u2', 'notification.friend.request']]);
  assert.deepEqual(sent.calls.pushes.map((p) => p.slice(0, 3)), [['u2', 'friend.request', 'Новая заявка в друзья']]);

  const crossing = harness({ async sendRequest() { return { status: 'accepted', user: { id: 'u2' } }; } });
  assert.equal((await crossing.service.sendRequest(ME, { userId: 'x', login: '' })).status, 'accepted');
  assert.deepEqual(crossing.calls.events, [['u2', 'friend-accepted'], ['u2', 'notification.friend.accepted']]);
  assert.equal(crossing.calls.pushes[0][2], 'Заявка принята');

  for (const status of ['not_found', 'self', 'blocked']) {
    assert.deepEqual(await harness({ async sendRequest() { return { status }; } }).service.sendRequest(ME, { userId: 'x', login: '' }), { status });
  }
  for (const status of ['already_friends', 'already_sent']) {
    const quiet = harness({ async sendRequest() { return { status, user: { id: 'u2' } }; } });
...
test('responding, cancelling, removing, blocking and unblocking', async () => {
  ...
  assert.equal((await harness({ async respondRequest() { return { status: 'not_found' }; } }).service.respond(ME, 'r', 'accept')).status, 'not_found');
  assert.equal((await harness({ async respondRequest() { return { status: 'blocked' }; } }).service.respond(ME, 'r', 'accept')).status, 'blocked');

  assert.equal((await service.cancel('user-1', 'r1')).status, 'cancelled');
  assert.deepEqual(calls.events.at(-1), ['u2', 'friend-request']);
  assert.equal((await harness({ async cancelRequest() { return { status: 'not_found' }; } }).service.cancel('u', 'r')).status, 'not_found');

  assert.equal((await service.remove('user-1', 'f1')).status, 'removed');
  assert.deepEqual(calls.events.at(-1), ['f1', 'friend-removed']);
  assert.equal((await harness({ async removeFriend() { return { status: 'not_found' }; } }).service.remove('u', 'f')).status, 'not_found');

  assert.deepEqual(await service.block('user-1', 'f1'), { status: 'applied', result: 'blocked' });
  assert.deepEqual(calls.events.at(-1), ['f1', 'friend-removed']);
  const stranger = harness({ async blockUser() { return { status: 'blocked', unfriended: false }; } });
  await stranger.service.block('user-1', 's');
  assert.deepEqual(stranger.calls.events, []);
  assert.equal((await harness({ async blockUser() { return { status: 'not_found' }; } }).service.block('u', 't')).status, 'not_found');
  assert.equal((await harness({ async blockUser() { return { status: 'invalid' }; } }).service.block('u', 't')).status, 'invalid');

  assert.equal((await service.unblock('user-1', 'b1')).status, 'unblocked');
  assert.equal((await harness({ async unblockUser() { return { status: 'not_found' }; } }).service.unblock('u', 'b')).status, 'not_found');
});

test('ringing refuses strangers, blocks, deleted accounts, cooldowns and missing rooms', async () => {
  assert.equal((await harness({ async areFriends() { return false; } }).service.ring(ME, 'room-1', 'f1')).status, 'not_friends');
  assert.equal((await harness({ async isBlockedBetween() { return true; } }).service.ring(ME, 'room-1', 'f1')).status, 'blocked');
  assert.equal((await harness({}, { deletedTarget: true }).service.ring(ME, 'room-1', 'f1')).status, 'account_deleted');
  assert.deepEqual(await harness({}, { rate: { allowed: false, retryAfterSeconds: 9 } }).service.ring(ME, 'room-1', 'f1'), { status: 'rate_limited', retryAfterSeconds: 9 });
  assert.deepEqual(await harness({}, { rate: { allowed: false } }).service.ring(ME, 'room-1', 'f1'), { status: 'rate_limited', retryAfterSeconds: 0 });
...
test('a ring notifies, files a room invitation DM for both sides and pushes until it expires', async () => {
  ...
  assert.deepEqual(calls.events, [['f1', 'ring.incoming'], ['f1', 'dm-message'], ['user-1', 'dm-message']]);
...
test('every social route needs a session and answers it', async (t) => {
  ...
  assert.deepEqual((await call(app, 'POST', `/api/friends/requests/${REQUEST_ID}/decline`, undefined)).body, { ok: true, status: 'accepted', user: { id: 'u2' } });
});

test('social refusals keep their texts and codes', async (t) => {
  const cases = [
    ['POST', '/api/friends/requests', {}, {}, 400, 'Неверный пользователь'],
    ['POST', '/api/friends/requests', { userId: FRIEND_ID }, { sendRequest: { status: 'not_found' } }, 404, 'Пользователь не найден'],
    ['POST', '/api/friends/requests', { userId: FRIEND_ID }, { sendRequest: { status: 'self' } }, 400, 'Нельзя добавить себя'],
    ['POST', '/api/friends/requests', { userId: FRIEND_ID }, { sendRequest: { status: 'blocked' } }, 403, 'Заявку отправить нельзя'],
    ['POST', '/api/friends/requests/bad/accept', undefined, {}, 404, 'Заявка не найдена'],
    ['POST', `/api/friends/requests/${REQUEST_ID}/accept`, undefined, { respond: { status: 'not_found' } }, 404, 'Заявка не найдена'],
    ['POST', `/api/friends/requests/${REQUEST_ID}/accept`, undefined, { respond: { status: 'blocked' } }, 409, 'Заявка больше недоступна'],
    ['DELETE', '/api/friends/requests/bad', undefined, {}, 404, 'Заявка не найдена'],
    ['DELETE', `/api/friends/requests/${REQUEST_ID}`, undefined, { cancel: { status: 'not_found' } }, 404, 'Заявка не найдена'],
    ...
    ['PUT', `/api/blocks/${FRIEND_ID}`, undefined, { block: { status: 'invalid' } }, 400, 'Нельзя заблокировать этого пользователя'],
    ['DELETE', '/api/blocks/bad', undefined, {}, 404, 'Пользователь не найден'],
    ['DELETE', `/api/blocks/${FRIEND_ID}`, undefined, { unblock: { status: 'not_found' } }, 404, 'Пользователь не заблокирован'],
    ...
    ['POST', '/api/rooms/room-1/ring', { userId: FRIEND_ID }, { ring: { status: 'not_friends' } }, 403, 'You are not friends'],
    ['POST', '/api/rooms/room-1/ring', { userId: FRIEND_ID }, { ring: { status: 'blocked' } }, 403, 'Invite is unavailable'],
    ['POST', '/api/rooms/room-1/ring', { userId: FRIEND_ID }, { ring: { status: 'account_deleted' } }, 403, 'Invite is unavailable'],
    ['POST', '/api/rooms/room-1/ring', { userId: FRIEND_ID }, { ring: { status: 'room_not_found' } }, 404, 'Room not found']
  ];
  for (const [method, url, payload, outcomes, status, error] of cases) {
    ...
    assert.deepEqual([response.status, response.body.error], [status, error], `${method} ${url} ${JSON.stringify(outcomes)}`);
  }
  const selfBlock = await call(routeApp(t).app, 'PUT', '/api/blocks/user-1');
  assert.equal(selfBlock.status, 400);
  const quiet = await call(routeApp(t, { sendRequest: { status: 'already_sent', user: { id: 'u2' } } }).app, 'POST', '/api/friends/requests', { userId: FRIEND_ID });
  assert.deepEqual([quiet.status, quiet.body], [200, { ok: true, status: 'already_sent', user: { id: 'u2' } }]);
  const declined = await call(routeApp(t, { respond: { status: 'declined' } }).app, 'POST', `/api/friends/requests/${REQUEST_ID}/decline`);
  assert.deepEqual(declined.body, { ok: true, status: 'declined' });
  const cooldown = await call(routeApp(t, { ring: { status: 'rate_limited', retryAfterSeconds: 4 } }).app, 'POST', '/api/rooms/room-1/ring', { userId: FRIEND_ID });
  assert.deepEqual([cooldown.status, cooldown.retryAfter, cooldown.body], [429, '4', { ok: false, error: 'Invite cooldown', retryAfterSeconds: 4 }]);
  const tooMany = await call(routeApp(t, {}, { limited: true }).app, 'POST', '/api/friends/requests', { userId: FRIEND_ID });
