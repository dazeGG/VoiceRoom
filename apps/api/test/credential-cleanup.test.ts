...
test('issued admission cleanup failure is logged, metered and preserves the primary failure', async () => {
  ...
  await assert.rejects(
    revokeIssuedAdmission({ boundary: { async revokeCredential() { throw cleanup; } }, cause: primary, credentialId: 'credential', principal: { principalId: 'u' }, recordFailure() { failures += 1; }, log: { error(value, message) { logs.push([value, message]); } }, roomId: 'room' }),
