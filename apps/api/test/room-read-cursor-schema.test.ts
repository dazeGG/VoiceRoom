...
test(
  'room and DM reads by cursor are stored and cover the microsecond-exact newest message',
  ...
  async (t) => {
    ...
    await pool.query(
      `INSERT INTO users (id, login, display_name, password_hash) VALUES ('me', 'readme', 'Me', 'x'), ('peer', 'readpeer', 'Peer', 'x')`
    );
    await pool.query(`INSERT INTO rooms (id, creator_ip) VALUES ('room', '')`);
    await pool.query(
      `INSERT INTO room_messages (id, room_id, peer_id, name, text, created_at) VALUES ('r1', 'room', 'p', 'Peer', 'hi', $1)`,
      [at]
    );
    await pool.query(
      `INSERT INTO direct_messages (id, sender_id, recipient_id, body, created_at) VALUES ('d1', 'peer', 'me', 'hi', $1)`,
      [at]
    );

    const reads = createMessageReadRepository({ pool });
    const room = await reads.advanceRoom({
      roomId: 'room',
      userId: 'me',
      tuple: { id: 'r1', createdAtMicros: micros }
    });
    ...
    const stored = await pool.query<{ last_read_message_id: string }>(
      `SELECT last_read_message_id FROM room_chat_reads WHERE room_id = 'room' AND user_id = 'me'`
    );
    assert.equal(stored.rows[0]?.last_read_message_id, 'r1');
    assert.deepEqual(
      await reads.advanceRoom({ roomId: 'room', userId: 'me', tuple: { id: 'r1', createdAtMicros: micros } }),
      { unchanged: true }
    );
    ...
    assert.equal(
      await reads.advanceDm({
        peerId: 'peer',
        userId: 'me',
        tuple: { id: 'd1', createdAtMicros: String(Number(micros) - 456) }
      }),
      null
    );
  }
