...
test(
  'G81-A01 room/DM bindings preserve order, owner/context and bind once',
    ...
    assert.deepEqual(
      (
        await repository.bindReady({
          ...
          attachmentIds: roomIds
        })
      ).map((item) => item.order),
      [0, 1]
    );
    await assert.rejects(
      repository.bindReady({ ownerId: 'owner', context: 'room', messageId: 'room-message', attachmentIds: roomIds }),
      /cannot be bound/
    );
...
test(
  'G81-A02 invalid/cross-owner draft rolls the message UoW back and fallback remains safe',
  { skip: !process.env.TEST_DATABASE_URL, timeout: 120000 },
  async (t) => {
    const { pool, repository } = await setup(t);
    ...
    try {
      ...
      await assert.rejects(
        repository.bindReady(
          { ownerId: 'owner', context: 'room', messageId: 'rolled-back', attachmentIds: [foreign] },
          client
        ),
        /cannot be bound/
      );
    ...
    assert.equal(
      (await pool.query(`SELECT count(*)::int count FROM room_messages WHERE id='rolled-back'`)).rows[0].count,
      0
    );
    assert.equal(
      (await pool.query(`SELECT text FROM room_messages WHERE id='room-message'`)).rows[0].text,
      '[Изображения: 2]'
    );
  }
);
