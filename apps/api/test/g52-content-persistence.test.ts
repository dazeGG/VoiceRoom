...
test('G52-A02 migration is nullable bounded and lock-limited', () => {
  const s = fs.readFileSync(
    path.resolve(import.meta.dirname, '../src/migrations/20260718131000_add_room_message_structured_content.cjs'),
    'utf8'
  );
  assert.match(s, /lock_timeout = '5s'/);
  assert.match(s, /LIMIT 1000/);
  assert.match(s, /content: \{ type: 'jsonb' \}/);
});
