...
function upgrade(port, requestPath) {
  return new Promise((resolve, reject) => {
...
test('G48-A04 real credential boundary survives restart, supports multitab, and rejects replay, tamper and ban-during-mint', async () => {
  ...
  const store = {
    ...
    async createLiveKitGateCredential(input) {
      if (revokeDuringCreate) epochs.set(key(input.principal, input.roomId), input.principalEpoch + 1);
  ...
  assert.equal(
    (await restartedGate.authorize(`/rtc?vr_gate_credential=${encodeURIComponent(first.credential.value)}`)).ok,
    true
  );
  assert.equal(
    (await restartedGate.authorize(`/rtc?vr_gate_credential=${encodeURIComponent(second.credential.value)}`)).ok,
    true
  );
  ...
  assert.equal(
    (await restartedGate.authorize(`/rtc?vr_gate_credential=${encodeURIComponent(raced.credential.value)}`)).ok,
    false
  );
});
...
test(
  ...
  async (t) => {
    ...
    const principal = { principalType: 'account', principalId: 'g48-account' };
    const boundary = createCredentialBoundaryService({ roomStore: store, secret: process.env.LIVEKIT_GATE_SECRET });
    ...
    const accessTokenFor = (credential) => {
      const claims = JSON.parse(
        Buffer.from(String(credential).split('.')[1] || '', 'base64url').toString('utf8') || '{}'
      );
    ...
    assert.match(await upgrade(gatePort, pathFor(issued[0].credential.value)), /^HTTP\/1\.1 101/);
    assert.match(await upgrade(upstreamPort, pathFor(issued[0].credential.value)), /^HTTP\/1\.1 403/);
    await close(gate);
    gate = makeGate();
    gatePort = await listen(gate);
    t.after(() => close(gate));
    const concurrent = await Promise.all(issued.map((entry) => upgrade(gatePort, pathFor(entry.credential.value))));
    assert.ok(concurrent.every((response) => /^HTTP\/1\.1 101/.test(response)));
    const tampered = `${issued[0].credential.value.slice(0, -1)}x`;
    ...
    for (const entry of issued)
      assert.match(await upgrade(gatePort, pathFor(entry.credential.value)), /^HTTP\/1\.1 403/);
    ...
    if (raced.status === 'issued')
      assert.match(await upgrade(gatePort, pathFor(raced.credential.value)), /^HTTP\/1\.1 403/);
    await close(gate);
    gate = makeGate();
    gatePort = await listen(gate);
    assert.match(await upgrade(gatePort, pathFor(issued[0].credential.value)), /^HTTP\/1\.1 403/);
    assert.match(
...
test(
  ...
  async (t) => {
    ...
    const principal = { principalType: 'account', principalId: 'g48-multitab-account' };
    const boundary = createCredentialBoundaryService({ roomStore: store, secret: process.env.LIVEKIT_GATE_SECRET });
    const healthy = await boundary.issueCredential({ roomId, peerId: 'healthy-tab', principal });
    assert.equal(healthy.status, 'issued');
    ...
    const app = createApiApp({
      ...
      liveKitCredentials: {
        async issueAdmission(input) {
          ...
          failedCredential = issued.credential;
          return {
            status: 'issued',
            admission: {
    ...
    assert.equal(
      (await gate.authorize(`/rtc?vr_gate_credential=${encodeURIComponent(healthy.credential.value)}`)).ok,
      true
    );
    assert.equal(
    ...
    const another = await boundary.issueCredential({ roomId, peerId: 'another-tab', principal });
    assert.equal(
      (await gate.authorize(`/rtc?vr_gate_credential=${encodeURIComponent(another.credential.value)}`)).ok,
      true
    );
...
test('G48-A03 issued credential fails closed when membership persistence and revoke both fail', async (t) => {
  ...
  const store = {
    ...
    async createRoom({ roomId, creatorIp, isStatic, name = '', now = Date.now() }) {
      const room = {
        id: roomId,
        creatorIp,
        isStatic,
