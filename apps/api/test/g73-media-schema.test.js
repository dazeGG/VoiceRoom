...
const BYTE_LIMIT_ROLLBACK_COUNT = (() => {
  const files = fs.readdirSync(path.resolve(import.meta.dirname, '../src/migrations')).filter((name) => name.endsWith('.cjs')).sort();
...
test('G73-A01 fresh PG schema is repeatable, bounded and N-1-readable', { skip: !process.env.TEST_DATABASE_URL, timeout: 120000 }, async (t) => {
  ...
  const migration = fs.readFileSync(fileURLToPath(new URL('../src/migrations/20260720162000_limit_message_attachment_bytes.cjs', import.meta.url)), 'utf8');
...
test('G73-A03 upgrade remediates historical 10-20MiB rows before validation and remains rollback safe', { skip: !process.env.TEST_DATABASE_URL, timeout: 120000 }, async (t) => {
  ...
  await runner({ databaseUrl: db.databaseUrl, dir: path.resolve(import.meta.dirname, '../src/migrations'), direction: 'down', count: BYTE_LIMIT_ROLLBACK_COUNT, migrationsTable: 'pgmigrations', logger: SILENT, noLock: true });
  ...
  await runner({ databaseUrl: db.databaseUrl, dir: path.resolve(import.meta.dirname, '../src/migrations'), direction: 'up', count: 1, migrationsTable: 'pgmigrations', logger: SILENT, noLock: true });
