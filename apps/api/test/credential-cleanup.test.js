...
test('issued admission cleanup failure is logged, metered and preserves the primary failure', async () => {
  ...
  await assert.rejects(
    revokeIssuedAdmission({ boundary: { async revokePrincipal() { throw cleanup; } }, cause: primary, principal: { principalId: 'u' }, recordFailure() { failures += 1; }, req: { log: { error(value, message) { logs.push([value, message]); } } }, roomId: 'room' }),
...
test('successful issued admission cleanup remains silent', async () => {
  ...
  await revokeIssuedAdmission({ boundary: { async revokePrincipal() { revoked += 1; } }, principal: { principalId: 'u' }, recordFailure() { failures += 1; }, roomId: 'room' });
