...
import { cleanPushSubscription, createNotificationSettingsService } from '../src/domains/notifications/notification-settings.service.ts';
...
function harness({ enabled = true, rate = { allowed: true }, upserted = true, result = null } = {}) {
  ...
  const store = {
    async getPreferences() { return { mutedPeerIds: [] }; },
    async setDmMute(input) { return { status: 'updated', preferences: { dm: input } }; },
    async setRoomMute(input) { return { status: 'updated', preferences: { room: input } }; },
    async setPrivateNotifications(input) { return { status: 'updated', preferences: { privacy: input } }; },
    async setDoNotDisturb() { return updated; },
    async setPresenceStatus() { return updated; }
  };
  const service = createNotificationSettingsService({
    preferences: () => store,
    pushes: () => ({
      async upsert(input) { calls.upserts.push(input); return upserted; },
      async remove(input) { calls.removed.push(input); }
    }),
    ...
    broadcastProfileToFriends: async (user) => { calls.friends.push(user.presenceStatus); }
  });
...
test('presence changes normalise the status and reach devices and friends', async () => {
  ...
  assert.deepEqual((await dnd.service.setDoNotDisturb(ME, true)).preferences, { doNotDisturb: true, presenceStatus: 'dnd' });
...
test('mutes and privacy pass straight to the preference store', async () => {
  ...
  assert.deepEqual((await service.setDmMute('user-1', PEER, true)).preferences.dm, { userId: 'user-1', peerUserId: PEER, muted: true });
  assert.deepEqual((await service.setRoomMute('user-1', 'room-1', false)).preferences.room, { userId: 'user-1', roomId: 'room-1', muted: false });
  assert.deepEqual((await service.setPrivateNotifications('user-1', true)).preferences.privacy, { userId: 'user-1', privateNotifications: true });
...
test('push subscribe and unsubscribe', async () => {
  ...
  assert.deepEqual(await harness({ rate: { allowed: false, retryAfterSeconds: 7 } }).service.subscribe('u', SUBSCRIPTION, 'UA'), { status: 'rate_limited', retryAfterSeconds: 7 });
  assert.deepEqual(await harness({ rate: { allowed: false } }).service.unsubscribe('u', SUBSCRIPTION.endpoint), { status: 'rate_limited', retryAfterSeconds: 0 });
...
test('every settings route needs a session and answers it', async (t) => {
  ...
  assert.deepEqual((await call(app, 'PUT', `/api/notifications/dm/${PEER}/mute`, { muted: true })).body, { ok: true, muted: true, preferences: { a: 1 } });
  assert.deepEqual((await call(app, 'PUT', '/api/notifications/privacy', { privateNotifications: false })).body, { ok: true, preferences: { a: 1 } });
...
test('settings refusals keep their texts', async (t) => {
  ...
  const cases = [
    ...
    ['POST', '/api/presence/status', { status: 'dnd', automatic: true }, 400, 'automatic presence can only transition between online and away']
  ];
  for (const [method, url, payload, status, error] of cases) {
    const response = await call(app, method, url, payload);
    assert.deepEqual([response.status, response.body.error], [status, error], `${method} ${url} ${JSON.stringify(payload)}`);
  }
  const self = await call(routeApp(t, {}, { user: { id: PEER } }).app, 'PUT', `/api/notifications/dm/${PEER}/mute`, { muted: true });
  assert.deepEqual([self.status, self.body.error], [400, 'Invalid notification target']);
  for (const [status, code, error] of [['not_found', 404, 'Not found'], ['self', 400, 'Invalid notification target'], ['temporary_room', 403, 'Only saved rooms can be muted'], ['not_saved_room', 403, 'Room is not saved']]) {
    const response = await call(routeApp(t, { setRoomMute: { status } }).app, 'PUT', '/api/notifications/room/room-1/mute', { muted: true });
  ...
  const pushCases = [
    ...
    ['unsubscribe', { status: 'rate_limited', retryAfterSeconds: 3 }, 'DELETE', 429, 'Too many push subscription changes']
  ];
