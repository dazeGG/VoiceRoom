...
test(
  ...
  async (t) => {
    ...
    await pool.query(
      `INSERT INTO users(id,login,display_name,password_hash) VALUES ('a','aa','A','x'),('r','rr','R','x');INSERT INTO rooms(id,creator_ip) VALUES ('room','');INSERT INTO room_messages(id,room_id,text) VALUES ('m','room','x');INSERT INTO user_notifications(id,recipient_user_id,actor_user_id,room_id,source_message_id,reasons,revision) VALUES ('n','r','a','room','m',ARRAY['mention'],7)`
    );
    const repo = createInboxRepository({ pool });
    const before = await repo.unreadCount('r');
    await repo.markRead({ recipientUserId: 'r', notificationId: 'n' });
    const after = await repo.unreadCount('r');
...
test(
  'G59-A02 concurrent retract shares the account revision allocator with upsert and reads',
  { skip: !process.env.TEST_DATABASE_URL },
  async (t) => {
    const db = await createTestDatabase(t);
    await runMigrations({
    ...
    const pool = new Pool({ connectionString: db.databaseUrl, max: 6 });
    t.after(async () => {
      await pool.end();
      await db.cleanup();
    });
