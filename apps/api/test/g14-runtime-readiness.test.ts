...
test(
  'G14-A02 live worker and API heartbeats gate runtime readiness fail closed',
  {
  ...
  async (t) => {
    ...
    await repository.heartbeat({
      kind: 'worker',
      id: 'worker-set-1',
      capabilityTokens: Object.values(WORKER_CAPABILITIES).flat(),
      ready: true
    });
    await repository.heartbeat({
      kind: 'api',
    ...
    await provider.refresh();
    assert.equal(
      provider.getSnapshot().replicaConsensus,
      false,
      'manifest disagreement disables every public capability'
    );
