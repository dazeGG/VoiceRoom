...
function wait(ms: number, signal: AbortSignal): Promise<void> { return new Promise<void>((resolve,reject)=>{ if(signal.aborted) return reject(signal.reason); const timer=setTimeout(done,ms); function done(){signal.removeEventListener('abort',cancel);resolve();} function cancel(){clearTimeout(timer);reject(signal.reason);}; signal.addEventListener('abort',cancel,{once:true}); }); }
...
function createNotificationDeliveryWorker({ outbox, provider, batchSize=50, leaseMs=120000, renewMs=30000, idleMs=250, maxAttempts=8, logger=createLogger({ name: 'worker.notification-delivery' }), observeOldestPending=recordNotificationOldestPending }: {
  ...
  function observe(ok: boolean) {
    const cutoff=Date.now()-600000; outcomes.push({at:Date.now(),ok}); while((outcomes[0]?.at as number)<cutoff)outcomes.shift();
  ...
  async function processLease(guard: FenceGuard) {
    ...
    while(guard.isOwned()) {
      ...
      if(disabledReason){await store.recordHeartbeat({...lease,ready:false});await wait(idleMs,guard.signal);continue;}
      ...
      for(const job of jobs){
        ...
        try {
          ...
          if(ageMs>15*60*1000){disabledReason='oldest_pending_exceeded';await store.reschedule(job.eventId,lease,{delayMs:60000,error:new Error(disabledReason),maxAttempts});continue;}
          ...
          if(suppressed){await store.markSuppressed(job.eventId,lease);observe(true);continue;}
          const result=await push.deliver({...job,payload:{...(job.payload as Record<string, any>),body:current!.private_notifications?'Откройте VoiceRoom, чтобы посмотреть уведомление.':(job.payload as Record<string, any> | null)?.body}});
          guard.assertOwned();
          if(result?.suppressed) await store.markSuppressed(job.eventId,lease); else await store.markDelivered(job.eventId,lease);
          observe(true);
        } catch(error) {
          guard.assertOwned();
          await store.reschedule(job.eventId,lease,{delayMs:boundedBackoff(job.attempts,{baseMs:5000,maxMs:3600000,jitter:.2}),error,maxAttempts});
          observe(false);
  ...
  const runtime=createLeaseRuntime({identity:LEASE_IDENTITY,leaseMs,renewMs,idleMs,logger,acquire:(x)=>store.acquireLease(x),renew:(x)=>store.renewLease(x),release:(x)=>store.releaseLease(x),run:processLease,onHeartbeat:(x)=>{void store.recordHeartbeat(x).catch(()=>{});}});
...
async function main(env: NodeJS.ProcessEnv=process.env): Promise<void>{
  ...
  let stopping: Promise<void> | undefined; const stop=(): Promise<void>=>stopping||(stopping=worker.stop().finally(()=>pool.end())); process.once('SIGINT',()=>void stop());process.once('SIGTERM',()=>void stop()); try{await worker.start();}finally{await stop();}
}

if(import.meta.main) main().catch((error: unknown)=>{createLogger({name:'worker.notification-delivery'}).fatal({evt:LOG_EVENTS.WORKER_FAILED,worker:'notification-delivery',err:error},'notification delivery worker failed');process.exitCode=1;});
