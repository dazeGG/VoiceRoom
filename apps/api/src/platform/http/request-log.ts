...
export function requestRouteLabel(request: { routeOptions?: { url?: string }; routerPath?: string; url?: string }): string {
  return request.routeOptions?.url || request.routerPath || request.url || 'unknown';
}

export function createRequestLog({ clientIp, hashIp }: { clientIp(req: IncomingMessage): string; hashIp(ip: string): string }) {
