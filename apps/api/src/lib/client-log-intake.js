...
const CLIENT_LOG_LEVELS = Object.freeze(['debug', 'info', 'warn', 'error']);
...
function cleanLevel(value) {
  ...
  return CLIENT_LOG_LEVELS.includes(text) ? text : 'info';
}
...
function normalizeClientLogEvent(raw, now) {
...
// dropping them.
function normalizeClientLogBatch(body, { now = Date.now() } = {}) {
