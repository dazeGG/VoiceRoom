...
const Answer = Type.Object({ ok: Type.Literal(true), deletionScheduledFor: Scheduled }, { additionalProperties: Type.Unknown() });
const Refusal = Type.Object({ ok: Type.Literal(false), error: Type.String(), code: Type.Optional(Type.String()), deletionScheduledFor: Scheduled });
...
export function registerAccountRoutes(root: FastifyInstance, ctx: ApiContext, deps: AccountRoutesDeps): void {
  ...
  function openSession(reply: FastifyReply, status: 200 | 201, opened: OpenedSession, extra: Record<string, unknown> = {}) {
    return reply.code(status).header('Set-Cookie', deps.sessionCookie(opened.token)).send({ ok: true as const, user: opened.user, ...extra });
  }
  ...
  app.post('/api/auth/register', {
    ...
    schema: { body: Type.Object({ login: Field, displayName: Field, password: Field, passwordConfirm: Field }), response: Responses }
  }, async (request, reply) => {
    ...
    switch (result.status) {
      case 'invalid_login': return reply.code(400).send(failure('Логин: 3–32 символа, латиница, цифры, . _ -'));
      case 'invalid_password': return reply.code(400).send(SHORT_PASSWORD);
      case 'password_mismatch': return reply.code(400).send(failure('Пароли не совпадают'));
      case 'login_taken': return reply.code(409).send(failure('Этот логин уже занят'));
      case 'signed_in': return openSession(reply, 201, result);
    }
  });
  ...
  app.post('/api/auth/login', {
    ...
    const result = await account.login({ login: normalizeLogin(request.body.login), password: text(request.body.password), device: device(request) });
    switch (result.status) {
      case 'invalid_credentials': return reply.code(401).send(WRONG_CREDENTIALS);
      case 'throttled': return tooMany(reply, result.retryAfterSeconds);
      case 'deletion_pending':
        return reply.code(409).send({ ...failure('Аккаунт ожидает удаления', { code: 'account_deletion_pending' }), deletionScheduledFor: result.deletionScheduledFor });
      case 'signed_in': return openSession(reply, 200, result);
    }
  });
  ...
  app.post('/api/auth/recover', {
    ...
    const result = await account.recover({ login, code: request.body.code, newPassword: text(request.body.newPassword), device: device(request) });
    if (result.status === 'invalid_new_password') return reply.code(400).send(SHORT_PASSWORD);
    if (result.status === 'invalid_code') return reply.code(401).send(failure('Неверный логин или код восстановления'));
    return openSession(reply, 200, result, { recoveryCodes: { remaining: result.remaining } });
  });

  app.post('/api/auth/account/restore', {
    preValidation: optionalJsonBody,
    schema: { body: Type.Object({ login: Field, password: Field }), response: Responses }
  }, async (request, reply) => {
    ...
    const result = await account.restore({ login: normalizeLogin(request.body.login), password: text(request.body.password), device: device(request) }, request.log);
  ...
  app.post('/api/auth/password', {
    ...
    const result = await account.changePassword(me.user.id, text(request.body.currentPassword), text(request.body.newPassword));
    switch (result.status) {
      case 'invalid_new_password': return reply.code(400).send(SHORT_PASSWORD);
      case 'not_found': return reply.code(404).send(ACCOUNT_NOT_FOUND);
      case 'invalid_password': return reply.code(400).send(failure('Неверный текущий пароль'));
      case 'changed': return reply.header('Set-Cookie', deps.clearedSessionCookie()).send({ ok: true as const });
    }
  });
  ...
  app.delete('/api/auth/sessions/:sessionId', {
    ...
    if (result.status === 'current_session') return reply.code(400).send(failure('Чтобы завершить этот сеанс, выйдите из аккаунта'));
  ...
  for (const resolution of ['confirm', 'deny'] as const) {
    app.post(`/api/auth/login-alerts/:alertId/${resolution}`, {
      schema: { params: Type.Object({ alertId: Type.String() }), response: Responses }
    }, async (request, reply) => {
      const me = await signedIn(request, reply);
      if (!me) return reply;
      const params = request.params as { alertId: string };
      const result = await account.resolveLoginAlert(me.user.id, me.session.publicId, params.alertId, resolution === 'confirm' ? 'confirmed' : 'denied');
      if (result.status === 'not_found') return reply.code(404).send(failure('Вход не найден или на него уже ответили'));
  ...
  app.post('/api/auth/account/deletion', {
    preValidation: optionalJsonBody,
    schema: { body: Type.Object({ currentPassword: Field }), response: Responses }
  }, async (request, reply) => {
    const me = await signedIn(request, reply);
    ...
    switch (result.status) {
      case 'unavailable': return reply.code(503).send(DELETION_UNAVAILABLE);
      case 'invalid_password': return reply.code(400).send(WRONG_PASSWORD);
      case 'not_found': return reply.code(404).send(ACCOUNT_NOT_FOUND);
      case 'already_requested':
        return reply.code(409).send({ ...failure('Аккаунт уже ожидает удаления', { code: 'account_deletion_pending' }), deletionScheduledFor: result.deletionScheduledFor });
      case 'scheduled':
        return reply.header('Set-Cookie', deps.clearedSessionCookie()).send({ ok: true as const, deletionScheduledFor: result.deletionScheduledFor });
    }
  });
}
