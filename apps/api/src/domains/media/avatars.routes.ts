...
export async function readAvatarUpload(request: FastifyRequest): Promise<Buffer> {
  ...
  try {
    ...
    if (isTooLarge(cause)) throw httpError(413, TOO_LARGE);
    throw cause;
  }
  if (!part || part.fieldname !== 'avatar') {
    part?.file?.resume?.();
    throw httpError(400, 'Multipart field "avatar" is required');
  }
  try {
    const buffer = await part.toBuffer();
    if (part.file?.truncated) throw httpError(413, TOO_LARGE);
    return buffer;
  } catch (cause) {
    if (isTooLarge(cause)) throw httpError(413, TOO_LARGE);
    throw cause;
...
export function registerAvatarRoutes(root: FastifyInstance, ctx: ApiContext, { avatars, rooms, uploadLimiter }: AvatarRoutesDeps): void {
  ...
  app.post('/api/auth/avatar', { schema: { response: Responses } }, async (request, reply) => {
    ...
    if (result.status === 'not_found') return reply.code(404).send(failure('Аккаунт не найден'));
    return { ok: true as const, user: result.user };
  });

  app.delete('/api/auth/avatar', { schema: { response: Responses } }, async (request, reply) => {
    const user = await signedIn(request, reply);
  ...
  async function sendImage(reply: FastifyReply, open: Promise<unknown>, missing: string) {
    ...
    if (!stream) return reply.code(404).send(failure(missing));
