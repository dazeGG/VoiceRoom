...
export function registerRoomRoutes(root: FastifyInstance, ctx: ApiContext, deps: RoomRoutesDeps): void {
  ...
  const sessionUserId = async (raw: Parameters<ApiContext['resolveSession']>[0]) =>
    (await ctx.resolveSession(raw))?.user?.id ?? null;
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const created = await deps.rooms.createRoom({
        creatorIp: clientIp,
        isStatic,
        ownerId,
        name: cleanRoomName(request.body.name)
      });
    ...
    async (request, reply) => {
      ...
      if (LEGACY_STATE_MUTATION_FIELDS.some((field) => Object.hasOwn(body, field))) {
        return reply.code(409).send(
          failure('Peer state updates require the active WebSocket connection', {
