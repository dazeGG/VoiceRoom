import { buildMembershipEnvelope, normalizeMembershipRequest, type MembershipEnvelope } from '@voice-room/shared/membership';
...
type PresenceEntry = { inVoice?: boolean; voice?: boolean; roomId?: unknown; presenceStatus?: string; status?: string } | null | undefined;
...
export type DirectoryListing =
  ...
  | { status: 'ok'; envelope: MembershipEnvelope };

function presenceForUser(snapshot: PresenceSnapshot, userId: string): { inVoice: boolean; presenceStatus: 'dnd' | 'online' | 'afk' | 'offline' } {
  const byUserId = snapshot?.byUserId;
  const raw = byUserId instanceof Map
    ? byUserId.get(userId)
...
function createMemberDirectoryService({ membershipService, repository, cursorCodec, getPresenceSnapshot = () => null }: {
  ...
  async function list({ roomId, viewerUserId, cursor, limit, query }: {
    ...
    const after = request.cursor
      ? codec.decode(request.cursor, { purpose: CURSOR_PURPOSE, context })
      : null;
    ...
    const nextCursor = page.hasMore && last
      ? codec.encode({ purpose: CURSOR_PURPOSE, context, tuple: last.cursorTuple })
      : undefined;
    return {
      status: 'ok',
