...
const Answer = Type.Object(
  { ok: Type.Literal(true), message: Type.Optional(Type.Unknown()) },
...
export function registerDirectMessageRoutes(root: FastifyInstance, ctx: ApiContext, dms: DirectMessagesService): void {
  ...
  app.post(
    '/api/dm/:userId/invites/:messageId/respond',
    {
      ...
      schema: {
        ...
        body: Type.Object({ action: Type.Optional(Type.Unknown()) }),
        response: Responses
      }
    },
    async (request, reply) => {
      ...
      switch (result.status) {
        case 'not_found':
          return reply.code(404).send(failure('Приглашение не найдено'));
        case 'not_invited':
          return reply.code(403).send(failure('Отвечать может только приглашённый'));
        case 'room_gone':
          return reply.code(410).send(failure('Комната больше не существует'));
        case 'already_answered':
          return reply.code(409).send(failure('Приглашение уже обработано'));
        case 'answered':
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      switch (result.status) {
        ...
        case 'not_sender':
          return reply.code(403).send(failure('Можно редактировать только свои сообщения'));
        case 'invitation':
          return reply.code(403).send(failure('Приглашение нельзя редактировать'));
        case 'rate_limited':
          return tooMany(reply, result.retryAfterSeconds);
        case 'edited':
          return { ok: true as const, message: result.message };
  ...
  app.delete(
    '/api/dm/:userId/messages/:messageId',
    { schema: { params: MessageParams, response: Responses } },
    async (request, reply) => {
      const user = await signedIn(request, reply);
      if (!user) return reply;
      const peerId = cleanUuid(request.params.userId);
