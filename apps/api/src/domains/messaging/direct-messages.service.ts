...
interface Delivery {
  idempotency: {
    ...
    ): Promise<{ kind: string; ledgerKey?: string; response?: { body?: unknown } }>;
    complete(
      client: DbClient,
      key: string,
...
export interface DirectMessagesDeps {
  ...
  readService(): {
    advanceDm(input: { cursor: string; peerId: string; userId: string }): Promise<Record<string, unknown>>;
  };
  friends(): {
  ...
  replies(): {
    lockDirectTarget(input: { userId: string; peerId: string; messageId: string; client: DbClient }): Promise<unknown>;
  };
...
export function createDirectMessagesService(deps: DirectMessagesDeps) {
  ...
  > {
    ...
    const messages = await Promise.all(
      stored.map(async (message) =>
        deps.projectReply('dm', await deps.projectMedia('dm', message), { userId: user.id, peerId })
      )
    );
  ...
  async function send(
    ...
    | { status: 'sent'; message: DirectMessage }
    | RateLimited
    | Status<
        | 'not_friends'
        | 'blocked'
    ...
    const stored = await deps.messages().direct.sendMessage({
      ...
      beforeUnitOfWork:
        idempotencyKey && delivery
          ? async (client) => {
              ...
              if (reservation.kind === 'replay')
                return {
                  ...
                  message: (reservation.response?.body as { message?: DirectMessage } | undefined)
          ...
          : null,
      unitOfWork:
        attachmentIds.length > 0 || replyToMessageId || delivery
          ? async (client, inserted) => {
              if (replies) {
                const target = await replies.lockDirectTarget({
                  userId: sender.id,
                  peerId: recipientId,
              ...
              if (media) {
                await media.attachments.bindReady(
                  { ownerId: sender.id, context: 'dm', messageId: inserted.id, attachmentIds },
                  client
                );
              }
              if (delivery) {
  ...
  async function respondInvite(
    ...
    | { status: 'answered'; message: DirectMessage }
    | Status<'not_found' | 'not_invited' | 'room_gone' | 'already_answered'>
  > {
  ...
  async function edit(
    ...
    | { status: 'edited'; message: DirectMessage }
    | RateLimited
    | Status<'empty' | 'not_found' | 'not_sender' | 'invitation'>
  > {
