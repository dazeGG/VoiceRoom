...
export function cleanPushSubscription(value: unknown): PushSubscription | null {
  ...
  if (!endpoint || endpoint.length > 4096 || !p256dh || p256dh.length > 1024 || !auth || auth.length > 1024)
    return null;
...
export function createNotificationSettingsService(deps: NotificationSettingsDeps) {
  ...
  return {
    ...
    setDmMute: (userId: string, peerUserId: string, muted: boolean) =>
      deps.preferences().setDmMute({ userId, peerUserId, muted }),
    setRoomMute: (userId: string, roomId: string, muted: boolean) =>
      deps.preferences().setRoomMute({ userId, roomId, muted }),
    ...
    async subscribe(
      ...
      userAgent: string
    ): Promise<RateLimited | { status: 'disabled' | 'invalid' | 'conflict' | 'subscribed' }> {
      ...
      const stored = await deps
        .pushes()
        .upsert({ userId, subscription, metadata: { userAgent: userAgent.slice(0, 512) } });
