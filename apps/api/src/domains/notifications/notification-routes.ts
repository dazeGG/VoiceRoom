...
function registerNotificationRoutes({ app, service, resolveUser, enabled = () => true }: {
  ...
  app.get<InboxRoute>('/api/notifications/inbox', async (request, reply) => {
    ...
    if (!user) return;
    try {
      return send(reply, 200, await notifications.list({ userId: user.id, cursor: request.query?.cursor, limit: request.query?.limit }));
    } catch (error) {
      if ((error as { code?: unknown } | null | undefined)?.code === 'invalid_cursor') return send(reply, 400, { ok: false, code: 'invalid_cursor' });
      throw error;
    }
  });
  ...
  app.post<InboxRoute>('/api/notifications/inbox/read-all', async (request, reply) => {
    const user = await auth(request, reply);
    if (!user) return;
    return send(reply, 200, await notifications.markAllRead({ userId: user.id, through: request.body?.through || null }));
  });
  ...
  app.get<InboxRoute>('/api/notifications/room/:roomId/level', async (request, reply) => {
    const user = await auth(request, reply);
    if (!user) return;
    return send(reply, 200, { ok: true, level: await notifications.getRoomLevel({ userId: user.id, roomId: request.params.roomId }) });
  });

  app.put<InboxRoute>('/api/notifications/room/:roomId/level', async (request, reply) => {
    const user = await auth(request, reply);
    if (!user) return;
    const result = await notifications.setRoomLevel({ userId: user.id, roomId: request.params.roomId, level: request.body?.level });
