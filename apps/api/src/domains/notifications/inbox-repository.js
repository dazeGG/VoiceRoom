...
function createInboxRepository({ pool } = {}) {
  ...
  async function upsert({ recipientUserId, actorUserId, roomId, sourceMessageId, reasons, body = '', client } = {}) {
    const result = await dbFor(pool, client).query(
      `INSERT INTO user_notifications (id,recipient_user_id,actor_user_id,room_id,source_message_id,reasons,body)
  ...
  async function unreadCount(recipientUserId, { client } = {}) { const r=await dbFor(pool,client).query('SELECT count(*)::int count, coalesce(max(revision),0)::bigint revision FROM user_notifications WHERE recipient_user_id=$1 AND read_at IS NULL AND retracted_at IS NULL',[recipientUserId]); return { count:Number(r.rows[0]?.count||0), revision:Number(r.rows[0]?.revision||0) }; }
  async function markRead({ recipientUserId, notificationId, client } = {}) { const r=await dbFor(pool,client).query('UPDATE user_notifications SET read_at=coalesce(read_at,current_timestamp),updated_at=current_timestamp WHERE id=$1 AND recipient_user_id=$2 RETURNING *',[notificationId,recipientUserId]); return mapRow(r.rows[0]); }
  async function markAllRead({ recipientUserId, through = null, client } = {}) { const r=await dbFor(pool,client).query(`UPDATE user_notifications SET read_at=coalesce(read_at,current_timestamp),updated_at=current_timestamp WHERE recipient_user_id=$1 AND read_at IS NULL AND ($2::timestamptz IS NULL OR created_at <= $2)`,[recipientUserId,through]); return r.rowCount; }
  async function retractByMessage(sourceMessageId,{client}={}) { const r=await dbFor(pool,client).query(`UPDATE user_notifications SET retracted_at=coalesce(retracted_at,current_timestamp),body='',revision=revision+1,updated_at=current_timestamp WHERE source_message_id=$1 AND retracted_at IS NULL RETURNING *`,[sourceMessageId]); return r.rows.map(mapRow); }
  return { list, markAllRead, markRead, retractByMessage, unreadCount, upsert };
}
