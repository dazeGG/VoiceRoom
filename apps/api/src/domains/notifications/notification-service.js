...
function createNotificationService({ pool, inbox, mentions, eligibility, outbox, cursorCodec, notificationStore } = {}) {
  ...
  async function markRoomRead({userId,roomId,through=null}){if(!userId||!roomId)return {ok:false,code:'invalid_request'};const result=await inbox.markReadForRoom({recipientUserId:userId,roomId,through});const unread=await inbox.unreadCount(userId);return {ok:true,updated:result?.updated||0,unreadCount:unread.count,revision:Math.max(unread.revision,Number(result?.revision)||0)};}
