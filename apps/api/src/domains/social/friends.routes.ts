...
export function registerFriendsRoutes(
  ...
  ctx: ApiContext,
  { friends, requestLimiter }: FriendsRoutesDeps
): void {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      switch (result.status) {
        ...
        default:
          return { ok: true as const, status: result.status, user: result.user };
      }
    }
  );
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      if (!user) return reply;
      const id = cleanUuid(request.params.id);
      if (!id || (await friends.cancel(user.id, id)).status === 'not_found')
        return reply.code(404).send(REQUEST_NOT_FOUND);
    ...
    async (request, reply) => {
      ...
      switch (result.status) {
        ...
        case 'account_deleted':
          return reply.code(403).send(failure('Invite is unavailable', { code: 'account_deleted' }));
        case 'rate_limited':
          return reply
            .code(429)
