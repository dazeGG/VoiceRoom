...
function toneless(sequence: string): string {
  let base = '';
...
function tonesIn(sequence: string): string[] {
  const tones: string[] = [];
  for (const character of sequence) {
...
function build(): { byBase: Map<string, ToneFamily>; collapsed: Set<string>; visible: readonly string[] } {
  const corpus = listReactionEmojis();
  const present = new Set(corpus);

  const families = new Map<string, Array<{ sequence: string; tones: string[] }>>();
  ...
  const byBase = new Map<string, ToneFamily>();
  ...
  for (const [base, variants] of families) {
    ...
    for (const { sequence, tones } of variants) {
      const [first] = tones;
      if (first !== undefined && tones.every((tone) => tone === first)) uniform.set(first, sequence);
    }
...
function data(): ReturnType<typeof build> {
  cached = cached || build();
  return cached;
}
