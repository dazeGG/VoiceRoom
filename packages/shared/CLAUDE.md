...
- Every module is one strict TypeScript source (`src/<name>.mts`, exported directly; see `tsconfig.json`: no Node types, the code also runs in the browser). API and Web import it through the package export; tests may `require()` or `import()` it. Do not add CommonJS, ESM or declaration copies.
- `src/emoji.mts` is the frozen Unicode corpus; G07 asserts its content hash, so do not edit the list.
