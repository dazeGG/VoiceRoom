...
test('G67-A01 canonical RGI desired-state and bigint summary contracts', async () => {
  ...
  assert.deepEqual(reactionContract.normalizeReactionSummary({ emoji: '😀', count: 0, reactedByMe: false, revision: '0' }), {
    emoji: '😀', count: 0, reactedByMe: false, revision: '0'
  });
  assert.equal(reactionContract.normalizeReactionSummary({ emoji: '😀', count: -1, reactedByMe: false, revision: '1' }), null);
});
...
test('G67-A02 reactor envelopes enforce default50/max100, opaque cursors and type parity', async () => {
  ...
  assert.deepEqual(reactionContract.normalizeReactorQuery({ limit: 100, cursor: 'opaque.cursor' }), { cursor: 'opaque.cursor', limit: 100 });
  assert.deepEqual(reactionContract.normalizeReactorQuery({ cursor: 'x'.repeat(4096) }), { cursor: 'x'.repeat(4096), limit: 50 });
