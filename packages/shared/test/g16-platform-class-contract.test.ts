...
const CORPUS = Object.freeze([
  ...
  },
  {
    ...
    input: { userAgent: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15)', platform: 'MacIntel', maxTouchPoints: 5 },
    expected: 'mobile'
  },
  {
    name: 'narrow Windows desktop',
    input: { userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', platform: 'Win32', viewportWidth: 320 },
    expected: 'desktop'
  },
  {
    name: 'UA-CH desktop',
    input: { userAgentDataMobile: false, userAgent: 'malformed mobile token' },
    expected: 'desktop'
  },
  { name: 'UA-CH mobile', input: { userAgentDataMobile: true, userAgent: 'Windows NT 10.0' }, expected: 'mobile' },
  {
    name: 'VoiceRoomDesktop bridge',
    input: { desktopBridge: true, userAgentDataMobile: true, userAgent: 'iPhone' },
    expected: 'desktop'
  },
  { name: 'unknown', input: {}, expected: 'unknown' },
  {
    name: 'malformed',
    input: { userAgent: { raw: 'iPhone' }, platform: 42, maxTouchPoints: 'five' },
    expected: 'unknown'
  }
]);
...
test('G16-A02 declaration and runtime contracts expose the same normalized DTO', () => {
  ...
  const exact: Record<keyof platform.PlatformPolicy, true> = {
