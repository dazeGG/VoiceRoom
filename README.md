...
- Голос через LiveKit; legacy WebRTC signaling остается только как fallback-инфраструктура.
...
Запустите LiveKit в dev-режиме:
...
Откройте `http://localhost:3000`. `localhost` считается безопасным browser context, поэтому микрофон и screen capture работают без HTTPS.
