...
Простой голосовой чат по ссылке. Комната живет в URL вида `/r/<room-id>`, сигналинг работает на Node.js без npm-зависимостей, звук идет через WebRTC напрямую между браузерами.
...
Откройте `http://localhost:3000`. Для доступа к микрофону на сервере нужен HTTPS; `localhost` браузеры считают безопасным контекстом.
