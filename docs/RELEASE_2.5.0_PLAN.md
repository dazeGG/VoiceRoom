...
**Status:** implementation executed under the user-authorized consolidated integration amendment; PR #114 is pending exact-head code and architecture approval for `develop`.
...
**Authority:** `docs/releases/2.5.0/amendments/CONSOLIDATED-INTEGRATION.json` authorizes the single consolidated implementation branch and PR to `develop`. It does not authorize a release branch, `main`, tag, GitHub Release, production promotion, provider/topology purchase, or dependency addition. Those gates remain separate and fail closed.
...
> **Consolidated integration amendment (2026-08-07):** the user subsequently directed the already-implemented B01–B09 sequence to be completed on one branch without per-card or per-subsystem PRs. `docs/releases/2.5.0/amendments/CONSOLIDATED-INTEGRATION.json` records that scope override, the exact merge base and PR, the retained goal/checkpoint obligations, the live-heartbeat readiness correction, the generalized exact-head evidence producer, and the G03 descendant-publication recovery. Develop merge still requires code-reviewer `APPROVE` and architect `CLEAR` on one exact head; any mutation restarts both lanes.
...
- Local planning-workspace artifacts are planning inputs; tracked docs and externally addressable immutable bundles/envelopes are authoritative during execution.
...
- **Read-only literal paths:** `.gitignore`; `docs/GIT_FLOW.md`.
...
- **Revision 8 — 2026-08-07 consolidated integration authority:** recorded the user's one-branch implementation override and PR #114 develop-only authority; restored every inherited migration byte and moved lock fencing into the migration runner; added live PostgreSQL worker/API heartbeat consensus; replaced impossible later-evidence bindings to the G01/G02-only archive workflow with a generalized exact-head, pinned-ORAS, attested producer; and allowed the frozen G03 terminal SHA only as an authenticated ancestor of the exact dispatched `develop` head. All RC, release, tag, production and external-observation gates remain pending.
...
- **Develop merge gate:** implementation authority comes from the explicit user scope override recorded in the consolidated amendment. PR #114 may merge only after independent code-reviewer `APPROVE` and architect `CLEAR` on the same exact head, with green repository/GitHub gates. Review evidence is recorded on the immutable PR head rather than self-declared inside that head. RC/release authority remains absent.
