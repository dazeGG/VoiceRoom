...
| Module system | The API package is ES modules (`"type": "module"`) from PR 0 on. Applied migrations are byte-pinned by `test/migration-history-integrity.test.ts`, so they stay CommonJS as `.cjs` files (same bytes, same migration names); new migrations are ES modules. |
