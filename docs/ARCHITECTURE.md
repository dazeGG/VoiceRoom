...
- PR 10k (next): remove the `bridge()` calls by tightening those domain contracts, then convert the tests (shared, web, API) and `scripts/`, and decide the audio worklets.
