...
**2b — меню устройств/вывода + status-pill (осталось)**
- status-pill (`ui/status.ts`, 165) → реактивный компонент от `state.serverConnection`/`voiceConnection`.
...
- **Риск:** низкий-средний (devices — списки + выбор, ближе к Фазе 3 по характеру).
- **Verify:** check + test + build + e2e (mute/devices), ручная проверка меню.
