...
**2b-ii — меню устройств/вывода (осталось)**
- `ui/devices.ts` (349), `ui/controls.ts` (115) → реактивные компоненты поверх
  `microphone-service`/`media-playback-service`.
- **Риск:** низкий-средний (списки устройств + выбор, ближе к Фазе 3 по характеру).
