...
Статус: реализовано 2026-07-11 в ветке `worktree-room-fallback-refactor`, ожидает PR.
...
3. **Данные пресетов удаляются без сохранения.** Down-миграция восстанавливает колонки
   с дефолтами (значения не возвращаются — допустимо).
...
- Down: восстановить колонки с дефолтами (`headphones`/`blue`/`''`) и констрейнты.
