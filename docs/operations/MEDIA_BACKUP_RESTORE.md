...
The release 2.5.0 media backup unit is a PostgreSQL dump plus the matching immutable uploads tree. `create-coordinated-snapshot.mts` copies both into one directory and writes their sizes and SHA-256 hashes to `manifest.json`; a partial pair is not restorable.
...
```sh
node scripts/backup/create-coordinated-snapshot.mts --database /safe/input/database.dump --uploads /safe/input/uploads --catalog /safe/input/media-catalog.json --output /safe/archive/snapshot-001 --namespace staging-restore-drill-001
```
...
```sh
node scripts/restore/restore-coordinated-snapshot.mts --snapshot /safe/archive/snapshot-001 --allowed-root /tmp/voice-room-restore-drill --target /tmp/voice-room-restore-drill/restored --namespace staging-restore-drill-001
```
